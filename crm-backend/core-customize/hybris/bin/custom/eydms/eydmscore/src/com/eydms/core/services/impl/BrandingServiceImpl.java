...
public class BrandingServiceImpl implements BrandingService {
    ...
    public EyDmsCustomerModel getCounterDetailsForPointOfSaleNew(String searchKeyWord) {
        EyDmsCustomerModel counterDetailsForPointOfSale = brandingDao.getCounterDetailsForPointOfSaleNew(searchKeyWord);
        return counterDetailsForPointOfSale;
    }
    ...
    public boolean submitBrandingRequisition(BrandingRequestDetailsData brandingRequestDetailsData){
        ...
        if(brandingRequestDetailsData!=null) {
            if (brandingRequestDetailsData.getIsModifyRequest()!=null && brandingRequestDetailsData.getIsModifyRequest() && brandingRequestDetailsData.getRequisitionNumber() != null) {
                ...
                if ((user.getGroups().contains(userService.getUserGroupForUID(EyDmsCoreConstants.CUSTOMER.DEALER_USER_GROUP_UID))) || (user.getGroups().contains(userService.getUserGroupForUID(EyDmsCoreConstants.CUSTOMER.RETAILER_USER_GROUP_UID)))) {
            ...
            if(brandingRequestDetailsData.getCounterCode()!=null) {
                EyDmsCustomerModel eydmsCustomer = (EyDmsCustomerModel) getUserService().getUserForUID(brandingRequestDetailsData.getCounterCode());
                if (eydmsCustomer != null && eydmsCustomer.getCustomerNo() != null) {
                    brandRequestDetailsModel.setCounterErpCustNo(eydmsCustomer.getCustomerNo());
                }
            }
            if(brandingRequestDetailsData.getLocation()!=null){
                brandRequestDetailsModel.setLocation(brandingRequestDetailsData.getLocation());
            }
            if(brandingRequestDetailsData.getStartDate()!=null) {
                brandRequestDetailsModel.setStartDate(getParsedDates(brandingRequestDetailsData.getStartDate()));
            }
            if(brandingRequestDetailsData.getPlanningDateOfCompletion()!=null) {
                brandRequestDetailsModel.setPlanningDateOfCompletion(getParsedDates(brandingRequestDetailsData.getPlanningDateOfCompletion()));
            }
                ...
                if (brandingRequestDetailsData.getBrandingSiteType().equals(BrandingSiteType.POINT_OF_SALE.getCode()) || brandingRequestDetailsData.getBrandingSiteType().equals(BrandingSiteType.DEALER_COSTSHARING_BRANDING.getCode())) {
                    ...
                    if (territoriesForCustomer != null && !territoriesForCustomer.isEmpty()) {
                        ...
                        if(subAreaMasterModel!=null)
                        {
                            ...
                            if(brandingRequestDetailsData.getCounterCode()!=null) {
                                EyDmsCustomerModel eydmsCustomer = (EyDmsCustomerModel) getUserService().getUserForUID(brandingRequestDetailsData.getCounterCode());
                                brandRequestDetailsModel.setState(eydmsCustomer.getState());
                            }
                        }
                        }
                        }
            ...
            modelService.save(brandRequestDetailsModel);
                ...
                if(brandRequestDetailsModel.getRequisitionRaisedDate()!=null && !isModify) {
                    EyDmsBrandingProcessModel eydmsBrandingProcessModel = new EyDmsBrandingProcessModel();
                    eydmsBrandingProcessModel.setBrandingRequestDetails(brandRequestDetailsModel);
                    eventService.publishEvent(new EyDmsBrandingEvent(eydmsBrandingProcessModel));
                }
                else if(brandRequestDetailsModel.getRequisitionRaisedDate()!=null && isModify)
                {
                    if(brandRequestDetailsModel.getEyDmsBrandingProcess()!=null && brandRequestDetailsModel.getEyDmsBrandingProcess().iterator().hasNext()) {
                        EyDmsBrandingProcessModel eydmsBrandingProcess = brandRequestDetailsModel.getEyDmsBrandingProcess().iterator().next();
                        if(eydmsBrandingProcess!=null) {
                            eydmsBrandingProcess.setAction("MODIFY");
                            modelService.save(eydmsBrandingProcess);
                            modelService.refresh(eydmsBrandingProcess);
                            businessProcessService.triggerEvent(eydmsBrandingProcess.getCode() + "_" + WAITING_FOR_BRANDING_APPROVAL);
                        }
                    }
                }
            }
    ...
    private Date getParsedDates(String date) {
        Date startDate = null;
        if(date!=null) {
            try {
    ...
    public DropdownListData getBrandingSiteTypeDropDownList(String brandingSiteType) {
        ...
        if(brandingSiteType.equals(BrandingSiteType.POINT_OF_SALE.getCode())){
            ...
            List<DropdownData> dataList = out.stream().map(this::getDropdownData).collect(Collectors.toList());
            data.setDropdown(dataList);
        }
            ...
            List<DropdownData> dataList = dcs.stream().map(this::getDropdownData).collect(Collectors.toList());
            data.setDropdown(dataList);
        }
        else
        {
    ...
    private CatalogUnawareMediaModel createMediaFromFile(final String uid, final String documentType, final MultipartFile file) {
        ...
        final String mediaCode = documentType.concat(EyDmsCoreConstants.UNDERSCORE_CHARACTER).concat(uid).concat(EyDmsCoreConstants.UNDERSCORE_CHARACTER).concat(String.valueOf(currentTimeInMillis));
            ...
            if (data.getRequisitionNumber() != null) {
                ...
                try {
                    if(data.getDateOfCompletion()!=null) {
                        Date date = new SimpleDateFormat("yyyy-MM-dd").parse(data.getDateOfCompletion());
                        detailsModel.setDateOfCompletion(date);
                    }
                } catch (Exception e) {
                ...
                {
                    if(detailsModel.getEyDmsBrandingProcess()!=null && detailsModel.getEyDmsBrandingProcess().iterator().hasNext()) {
                        EyDmsBrandingProcessModel eydmsBrandingProcess = detailsModel.getEyDmsBrandingProcess().iterator().next();
                        if(eydmsBrandingProcess!=null) {
                            eydmsBrandingProcess.setAction("FEEDBACK");
                            modelService.save(eydmsBrandingProcess);
                            modelService.refresh(eydmsBrandingProcess);
                            businessProcessService.triggerEvent(eydmsBrandingProcess.getCode() + "_" + WAITING_FOR_BRANDING_APPROVAL);
                        }
    		...
    		try{
    			if(brandingRequestDetails!=null) {
    				...
    				if (status.equalsIgnoreCase("APPROVE")) {
    					...
    					EyDmsBrandingProcessModel eydmsBrandingProcessModel = new EyDmsBrandingProcessModel();
    					eydmsBrandingProcessModel.setBrandingRequestDetails(brandingRequestDetails);
    					eventService.publishEvent(new EyDmsBrandingEvent(eydmsBrandingProcessModel));
    					...
    					if(brandingRequestDetails.getEyDmsBrandingProcess()!=null && brandingRequestDetails.getEyDmsBrandingProcess().iterator().hasNext()) {
    						EyDmsBrandingProcessModel eydmsBrandingProcess = brandingRequestDetails.getEyDmsBrandingProcess().iterator().next();
    						if(eydmsBrandingProcess!=null) {
    							eydmsBrandingProcess.setAction("CANCEL");
    							modelService.save(eydmsBrandingProcess);
    							modelService.refresh(eydmsBrandingProcess);
    							businessProcessService.triggerEvent(eydmsBrandingProcess.getCode() + "_" + WAITING_FOR_BRANDING_APPROVAL);
    						}
            ...
            if (brandRequestDetailsModel != null) {
                ...
                if (brandingRequestDetailsData.getBrandingSiteType().equals(BrandingSiteType.POINT_OF_SALE.getCode()) ||
                    ...
                    if(brandingRequestDetailsData.getCounterCode()!=null && brandingRequestDetailsData.getSlctReqNo()!=null) {
                        EyDmsCustomerModel eydmsCustomer=slctCrmIntegrationService.findCustomerByCustomerNo(brandingRequestDetailsData.getCounterCode());
                        if(eydmsCustomer!=null && eydmsCustomer.getUid()!=null) {
                            brandingRequestDetailsData.setCounterCode(eydmsCustomer.getUid());
                            brandingRequestDetailsData.setCounterName(eydmsCustomer.getName());
                        }
                    }
