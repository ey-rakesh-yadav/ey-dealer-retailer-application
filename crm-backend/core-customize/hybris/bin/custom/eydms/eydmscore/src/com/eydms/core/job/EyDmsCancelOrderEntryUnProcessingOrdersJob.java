...
public class EyDmsCancelOrderEntryUnProcessingOrdersJob extends AbstractJobPerformable<CronJobModel> {

	private static final Logger LOG = Logger.getLogger(EyDmsCancelOrderEntryUnProcessingOrdersJob.class);
	...
	private void executeJob(String baseSiteId) {
		...
		for (OrderEntryModel orderEntryModel: orderEntryList) {
			try {
				eydmsB2BOrderService.cancelOrderEntryFromCRM(orderEntryModel, REASON, null, true);
				eydmsB2BOrderService.getRequisitionStatusByOrderLines(orderEntryModel.getOrder());
			}
			catch (Exception e) {
