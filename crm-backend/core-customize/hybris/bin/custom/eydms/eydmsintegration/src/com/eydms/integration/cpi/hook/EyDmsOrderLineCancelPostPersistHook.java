...
public class EyDmsOrderLineCancelPostPersistHook implements PostPersistHook {
    ...
    private void cancelOrder(OrderModel order) {
        ...
        if(null!=order) {
            ...
            notification.setSubject(EyDmsintegrationConstants.ORDER_LINE_CANCELLED_NOTIFICATION);
            builder.append(order.getPlacedBy().getUid()).append(ORDER_PLACED_BY);
            builder.append(order.getUser().getUid()).append(ORDER_DATE);
            DateFormat dateFormat = new SimpleDateFormat(EyDmsintegrationConstants.CUSTOMER_ACCOUNT.DATE_FORMAT_1);
