...
import com.eydms.core.services.impl.CollectionServiceImpl;
...
import com.eydms.facades.CollectionFacade;
...
public class CollectionFacadeImpl implements CollectionFacade{
	...
	public Boolean uploadSPInvoice(SPInvoiceData data) {
		...
		invoice.setSalesPromoter((EyDmsCustomerModel) userService.getCurrentUser());
		...
		{
			try {
				invoice.setInvoiceFile(eydmsMediaReverseConverter.convert(genericMediaUtil.getMultipartFile(data.getInvoiceFile().getByteStream(),data.getInvoiceFile().getFileName())));
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
	...
	public CollectionDealerOutstandingDetailsListData getDealerOutstandingDetailsForTSMRH(SearchPageData searchPageData,List<String> so,List<String> tsm) {
		...
		List<EyDmsUserModel> soList = new ArrayList<>();
		List<EyDmsUserModel> tsmList = new ArrayList<>();
		if(so!=null && !so.isEmpty()){
			for(String str : so){

				LOG.info("so: " + str);
				EyDmsUserModel user = (EyDmsUserModel) userService.getUserForUID(str);
				soList.add(user);
			}
		}
		...
		if(tsm!=null && !tsm.isEmpty()){
			for(String str1: tsm){
				LOG.info("tsm: " + tsm);
				EyDmsUserModel user1 = (EyDmsUserModel) userService.getUserForUID(str1);
				tsmList.add(user1);
			}
		}
		...
		if(searchResult!=null && searchResult.getResults()!=null) {
			...
			for (CreditAndOutstandingModel creditAndOutstandingModel : list) {
				...
				EyDmsCustomerModel eydmsCustomer = slctCrmIntegrationService.findCustomerByCustomerNo(creditAndOutstandingModel.getCustomerCode());
				EyDmsUserModel eydmsUser = territoryManagementService.getSOforCustomer(eydmsCustomer);
				if (eydmsUser != null) {
					collectionDealerOutstandingDetailsData.setSoName(eydmsUser.getName());
					collectionDealerOutstandingDetailsData.setSoEmployeeCode(eydmsUser.getEmployeeCode());
				}
					collectionDealerOutstandingDetailsData.setCounterName(eydmsCustomer.getName());
					...
					collectionDealerOutstandingDetailsData.setErpCode(eydmsCustomer.getCustomerNo());
					collectionDealerOutstandingDetailsData.setCounterCode(eydmsCustomer.getUid());
...
//					collectionDealerOutstandingDetailsData.setTsmEmployeeCode("TSM1");
  ...
  @Override
	...
	public CollectionOutstandingData getOutstandingDataForTSMRH() {
		...
		data.setTotalOutstanding(String.valueOf(totalOutstanding));
		return data;
	}
