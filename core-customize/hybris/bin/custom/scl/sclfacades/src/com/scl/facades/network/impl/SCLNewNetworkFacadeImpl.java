...
import com.scl.core.services.impl.NetworkServiceImpl;
...
public class SCLNewNetworkFacadeImpl implements SCLNewNetworkFacade {

    @Autowired
    SCLNewNetworkService sclNewNetworkService;
    @Autowired
    ...
    private static final Logger LOG = Logger.getLogger(SCLNewNetworkFacadeImpl.class);
    ...
    public List<InfluencerSummaryData> getInfluencerDetailedPaginatedSummaryList(RequestCustomerData requestCustomerData) {
        ...
        List<SclCustomerModel> influencerList = territoryManagementService.getCustomerforUser(requestCustomerData);
    ...
    public List<DealerCurrentNetworkData> getRetailedDetailedPaginatedSummaryList(RequestCustomerData requestCustomerData) {
        ...
        List<SclCustomerModel> retailerList = territoryManagementService.getCustomerforUser(requestCustomerData);
    ...
    public List<DealerCurrentNetworkData> getDealerDetailedSummaryList(RequestCustomerData requestCustomerData) {
        ...
        List<List<Object>> list = sclNewNetworkService.getDealerDetailedSummaryList(requestCustomerData);
        ...
        if(list!=null && !list.isEmpty()){
            dealerList = list.stream().filter(each -> each.get(0)!=null).map(each->(SclCustomerModel)each.get(0)).collect(Collectors.toList());
        }
        List<DealerCurrentNetworkData> summaryDataList = new ArrayList<>();
        if(dealerList!=null && !dealerList.isEmpty()){
           ...
           for (List<Object> subList : list){
                SclCustomerModel dealer = (SclCustomerModel) subList.get(0);
                ...
                double target =salesPerformanceService.getMonthlySalesTargetForDealer(dealer,baseSiteService.getCurrentBaseSite(),null);
                //double target = sclUserDao.getCustomerTarget(dealer.getUid(), SclDateUtility.getMonth(new Date()), SclDateUtility.getYear(new Date()));
    ...
    public Integer getProposalCount(String leadType) {
         ...
         if (currentUser.getUserType().getCode().equals("TSM")) {
             ...
             List<NetworkAdditionPlanModel> data = sclNewNetworkService.getProposedPlanSummaryList(leadType1, soForUser);
             if (data.size() > 0)
                 return data.size();
             else
                 return 0;
        ...
        if (currentUser.getUserType().getCode().equals("RH")) {
            ...
            List<SclUserModel> tsmForUser = territoryManagementService.getTSMForUser(filterDistrictData);
            LeadType leadType1 = LeadType.valueOf(leadType);
            List<NetworkAdditionPlanModel> data = sclNewNetworkService.getCountOfProposedPlanSummaryListForRH(leadType1, tsmForUser);
            if (data.size() > 0)
                return data.size();
            else
                return 0;
    ...
    public ProposePlanListData getProposedPlanSummaryList(LeadType leadType) {
        ...
        List<NetworkAdditionPlanModel> data = sclNewNetworkService.getProposedPlanSummaryList(leadType, soForUser);
    ...
    public ProposePlanListData getProposedPlansBySO(LeadType leadType, String filter) {
        ...
        List<NetworkAdditionPlanModel> data = sclNewNetworkService.getProposedPlansBySO(leadType, soForUser,filter);
        if(data!=null && !data.isEmpty())
        {
            for (NetworkAdditionPlanModel dataSet : data) {
                proposePlanData.setRevisionReason(dataSet.getReason());
    ...
    @Override
    public String getProposedPlanViewDetails(SCLNetworkAdditionPlanData planData) {
    ...
    public boolean updateTargetStatusForApprovalNwAddition(SCLNetworkAdditionPlanData salesApprovalData) {
        ...
        return sclNewNetworkService.updateTargetStatusForApprovalNwAddition(salesApprovalData);

    }

    @Override
    public boolean targetSendForRevisionNwAddition(ProposePlanData salesRevisedTargetData) {
        return sclNewNetworkService.targetSendForRevisionNwAddition(salesRevisedTargetData);
    }

    @Override
    public SCLNetworkAdditionPlanData proposedPlanViewForTSMRH(String status, String id) {
        SCLNetworkAdditionPlanData sclNetworkAdditionPlanData = new SCLNetworkAdditionPlanData();
        SclUserModel currentUser = (SclUserModel) userService.getCurrentUser();
        BaseSiteModel currentBaseSite = baseSiteService.getCurrentBaseSite();


        NetworkAdditionPlanModel networkAdditionPlanModel = sclNewNetworkService.getProposedPlanViewForTSMRH(status, id);
        if(networkAdditionPlanModel!=null){
            sclNetworkAdditionPlanData  = networkAdditionPlanConverter.convert(networkAdditionPlanModel, sclNetworkAdditionPlanData);
        ...
        /*if(currentUser.getUserType().getCode().equals("TSM")){
           ...
           // NetworkAdditionPlanModel networkAdditionPlanModel = sclNewNetworkService.getProposedPlanViewForTSMRH(status, id);
            ...
            if (networkAdditionPlanModel != null) {
                sclNetworkAdditionPlanData.setSubAreaName(networkAdditionPlanModel.getSubAreaMaster().getTaluka());
                sclNetworkAdditionPlanData.setSoName(networkAdditionPlanModel.getRaisedBy().getName());
                sclNetworkAdditionPlanData.setAdditionCount(networkAdditionPlanModel.getRevisedPlan());
                    //sclNetworkAdditionPlanData.setCity(networkAdditionPlanModel.getCity());
                sclNetworkAdditionPlanData.setTaluka(networkAdditionPlanModel.getTaluka());
                sclNetworkAdditionPlanData.setDistrictName(networkAdditionPlanModel.getDistrictMaster().getName());
                sclNetworkAdditionPlanData.setTotalCounter(networkAdditionPlanModel.getTotalCounter());
                sclNetworkAdditionPlanData.setShreeCounter(networkAdditionPlanModel.getShreeCounter());
                sclNetworkAdditionPlanData.setEnableApproveFormCompletion(Boolean.TRUE);
                sclNetworkAdditionPlanData.setEnableRevisedFormCompletion(Boolean.TRUE);
            ...
            NetworkAdditionPlanModel networkAdditionPlanModel = sclNewNetworkService.getProposedPlanViewForTSMRH(status, id);
            if (networkAdditionPlanModel != null){
                sclNetworkAdditionPlanData.setDistrict(networkAdditionPlanModel.getDistrict());
                sclNetworkAdditionPlanData.setDiName(networkAdditionPlanModel.getRevisedBy().getName());
                sclNetworkAdditionPlanData.setAdditionCount(networkAdditionPlanModel.getRevisedPlan());
                sclNetworkAdditionPlanData.setTaluka(networkAdditionPlanModel.getTaluka());
                sclNetworkAdditionPlanData.setDistrictName(networkAdditionPlanModel.getDistrictMaster().getName());
                sclNetworkAdditionPlanData.setTotalCounter(networkAdditionPlanModel.getTotalCounter());
        ...
        return sclNetworkAdditionPlanData;
    }
    ...
    public ProposePlanListData getProposedPlanSummaryListForTSMRH(SearchPageData searchPageData,LeadType leadType,List<String> statuses,boolean isPendingForApproval) {
        ...
        List<SclUserModel> soForUser = new ArrayList<>();
        List<SclUserModel> tsmForUser = new ArrayList<>();
        ...
        SearchPageData<NetworkAdditionPlanModel> searchResult = sclNewNetworkService.getProposedPlanSummaryListForTSMRH(searchPageData, leadType, statuses, isPendingForApproval, soForUser, tsmForUser, currentUser);
        ...
        if (currentUser.getUserType().getCode().equals("TSM")) {
            ...
            //    diApprovedAddition = sclNewNetworkService.getApprovedAdditionSumForTSMRH(leadType, soForUser, tsmForUser, currentUser);
            ...
            //    rhApprovedAddition = sclNewNetworkService.getApprovedAdditionSumForTSMRH(leadType, soForUser, tsmForUser, currentUser);
                ...
                if(leadType.equals(LeadType.RETAILER)){
                    ...
                    if(networkAdditionPlanModel!=null && !networkAdditionPlanModel.isEmpty()){
                        for(NetworkAdditionPlanModel planModel: networkAdditionPlanModel) {
                            sclWorkflowModel = planModel.getApprovalWorkflow();

                            if (sclWorkflowModel != null) {
                                planModel.setApprovalLevel(TerritoryLevels.REGION);
                                SclWorkflowModel approvalWorkflowModel = planModel.getApprovalWorkflow();
                                SclWorkflowActionModel sclWorkflowActionModel = sclWorkflowService.saveWorkflowAction(approvalWorkflowModel, "", planModel.getBrand(), planModel.getSubAreaMaster(), TerritoryLevels.REGION);
