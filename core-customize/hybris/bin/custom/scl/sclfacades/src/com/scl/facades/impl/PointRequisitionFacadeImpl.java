...
import com.scl.facades.PointRequisitionFacade;
...
public class PointRequisitionFacadeImpl implements PointRequisitionFacade {
    ...
    public List<InfluencerPointRequisitionRequestData> getAllocationRequestList() {
        ...
        if(allocationRequestList!=null && !allocationRequestList.isEmpty()) {
            Map<SclCustomerModel, List<PointRequisitionModel>> collect = allocationRequestList.stream().collect(groupingBy(PointRequisitionModel::getRequestRaisedFor));
            if(collect!=null && !collect.isEmpty()) {
                for (Map.Entry<SclCustomerModel, List<PointRequisitionModel>> sclCustomerModelListEntry : collect.entrySet()) {
                    SclCustomerModel key = sclCustomerModelListEntry.getKey();
                    ...
                    if(sclCustomerModelListEntry.getValue()!=null && !sclCustomerModelListEntry.getValue().isEmpty()) {
                        for (PointRequisitionModel pointRequisitionModel : sclCustomerModelListEntry.getValue()) {
    ...
    public SearchPageData<CustomerData> getList(String filter, SearchPageData searchPageData) {
        ...
        SearchPageData<SclCustomerModel> retailerListForDealer = pointRequisitionService.getList(filter,searchPageData);
        ...
        List<SclCustomerModel> results = retailerListForDealer.getResults();
    ...
    public SearchPageData<CustomerData> getSavedDealerRetailer(SearchPageData searchPageData) {
        final SearchPageData<CustomerData> result = new SearchPageData<>();
        SearchPageData<SclCustomerModel> retailerListForDealer = pointRequisitionService.getSavedDealerRetailer(searchPageData);
        result.setPagination(retailerListForDealer.getPagination());
        result.setSorts(retailerListForDealer.getSorts());
        List<CustomerData> sclCustomerData = dealerBasicConverter.convertAll(retailerListForDealer.getResults());
