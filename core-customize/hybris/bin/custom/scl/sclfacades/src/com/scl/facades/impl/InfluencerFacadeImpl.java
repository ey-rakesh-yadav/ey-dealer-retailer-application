...
import com.scl.core.enums.TypeOfVisitMaster;
...
import com.scl.core.enums.VenueType;
import com.scl.core.model.EndCustomerComplaintModel;
...
public class InfluencerFacadeImpl implements InfluencerFacade {
	...
	@Autowired
	Converter<AddressModel, AddressData> sclAddressConverter;
...
//		if(partner.getClass().equals(SclUserModel.class))
//		{
...
//		}
	...
	@Override
	public Boolean editProfile(SclCustomerData data) {
		SclCustomerModel customer = (SclCustomerModel) userService.getUserForUID(data.getUid());
		...
		if (CollectionUtils.isNotEmpty(list)) {
			...
			if (billingAddressList != null && !billingAddressList.isEmpty()) {
				AddressModel billingAddress = billingAddressList.get(0);
				if (null != billingAddress) {
					sclAddressReversePopulator.populate(data.getAddress(), billingAddress);
		...
		if (!Objects.isNull(data.getProfilePic())) {
			customer.setProfilePicture(sclMediaReverseConverter.convert(genericMediaUtil.getMultipartFile(data.getProfilePic().getByteStream(), data.getProfilePic().getFileName())));
		}
	...
	public OnboardingPartnerData fetchOnboardingPartnerDetails(String influencerUid) {
		SclCustomerModel customerModel = influencerService.fetchOnboardingPartnerDetails(influencerUid);
		OnboardingPartnerData data = new OnboardingPartnerData();
	...
	public CustomerCardListData getInfluencerOnboardingList(SearchPageData searchPageData,
		...
		SearchPageData<SclCustomerModel> customerList = influencerService.getInfluencerOnboardingList(searchPageData, customerRequestData);
	...
	@Override
	public SclLeadData updateLeadStatus(String leadId, String status, String rejectedComment) {
		SclLeadData leadData = new SclLeadData();
	...
	@Override
	public SclLeadData viewLeadDetailsById(String leadId) {
		SclLeadData leadData = new SclLeadData();
		LeadMasterModel leadMasterModel = networkService.findItemByUidParam(leadId);
		if (leadMasterModel != null) {
...
/*
	...
	private void updateTotalVisitDetails(SclCustomerModel influencer, SclUserModel user) {
		int currentYear = LocalDate.now().getYear();
		int currentMonth = LocalDate.now().getMonthValue();
	...
	public InfluencerVisitData saveInfluencerVisitForm(InfluencerVisitData influencerVisitData) {
		...
		SclCustomerModel influencer = (SclCustomerModel) userService.getUserForUID(influencerVisitData.getInfluencerId());
	...
	public InfluencerVisitMasterModel createCounterVisitMasterData(VisitMasterModel visit,SclCustomerModel influencer, InfluencerVisitData visitData) {
		...
		model.setSequence(1);
		model.setSclCustomer(influencer);
  ...
  @Override
    ...
    public InfluencersDetails360WsData getInfluencerDetailsFor360(String influencerCode) {
       ...
       if(lastMonthSale!=null && lastMonthSale.get(influencerCode)!=null) {
		...
		if(influencer.getSclCustomers()!=null && !influencer.getSclCustomers().isEmpty()) {
			List<B2BCustomerModel> siteList = influencer.getSclCustomers().stream().filter(o -> o.getGroups().contains(userService.getUserGroupForUID(SclCoreConstants.CUSTOMER.SITE_USER_GROUP_UID))).collect(Collectors.toList());
			if (siteList != null && !siteList.isEmpty()) {
				List<SclSiteData> dataList = sclSiteConverter.convertAll(siteList);
				data.setSites(dataList);
			}
		}
		/*if(influencer.getTaggedPartners()!=null && !influencer.getTaggedPartners().isEmpty()) {
			List<B2BCustomerModel> siteList = influencer.getTaggedPartners().stream().filter(o -> o.getGroups().contains(userService.getUserGroupForUID(SclCoreConstants.CUSTOMER.SITE_USER_GROUP_UID))).collect(Collectors.toList());
			if (siteList != null && !siteList.isEmpty()) {
				List<SclSiteData> dataList = sclSiteConverter.convertAll(siteList);
				data.setSites(dataList);
			}
		...
		if(StringUtils.isNotBlank(filter)){
			...
			if(filter.equalsIgnoreCase("Daily")){
				...
				for(int i=1;i<=7;i++){
					...
					influencerVisitHistoryCount = influencerService.getInfluencerVisitHistoryCount(day.toString(), day.plusDays(1).toString());
					...
					data.setDailyWeekMonthName(String.valueOf(day.getDayOfMonth()).concat("/").concat(String.valueOf(day.getMonthValue())));
					dataList.add(data);
					day = day.minusDays(1);
				}
