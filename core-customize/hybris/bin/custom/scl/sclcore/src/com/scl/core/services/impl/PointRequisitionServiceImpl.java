...
import com.scl.core.services.PointRequisitionService;
...
public class PointRequisitionServiceImpl implements PointRequisitionService {
   ...
   private DataConstraintDao dataConstraintDao;
      ...
      //  final List<ErrorWsDTO> errorWsDTOList = new ArrayList<>();
        ...
        if(Objects.nonNull(pointRequisitionData)) {
            ...
            if (Objects.nonNull(sclCustomer)) {
                pointRequisitionModel.setRequestRaisedBy(sclCustomer);
                if(sclCustomer.getCounterType()!=null && (sclCustomer.getCounterType().equals(CounterType.DEALER) || sclCustomer.getCounterType().equals(CounterType.RETAILER))) {
                	isDealerRetailer = true;
                }
            }
            //dealer or retailer
            if (pointRequisitionData.getRequestRaisedTo() != null) {
                SclCustomerModel dealerOrRetailer = (SclCustomerModel) userService.getUserForUID(pointRequisitionData.getRequestRaisedTo());
            ...
            if (pointRequisitionData.getRequestRaisedFor() != null) {
                SclCustomerModel influencer = (SclCustomerModel) userService.getUserForUID(pointRequisitionData.getRequestRaisedFor());
    ...
    @Override
    public void saveCustomerInfluencerMapping(SclCustomerModel fromCustomer, SclCustomerModel influencer, BaseSiteModel brand) {
        ...
        if(Objects.isNull(customersInfluencerMapModel)) {
            ...
            if(fromCustomer.getGroups().contains(userService.getUserGroupForUID(SclCoreConstants.CUSTOMER.DEALER_USER_GROUP_UID))) {
            ...
            else if (fromCustomer.getGroups().contains(userService.getUserGroupForUID(SclCoreConstants.CUSTOMER.RETAILER_USER_GROUP_UID))) {
...
//            modelService.save(customersInfluencerMapModel);
    ...
    @Override
    public void orderCountIncrementForCustomerRetailerMap(Date deliveryDate,SclCustomerModel fromCustomer, SclCustomerModel influencer, BaseSiteModel brand,Double qty) {
        CustomersInfluencerMapModel customersInfluencerMapModel = pointRequisitionDao.getInfluencersListForCustomers(fromCustomer, influencer, brand);
        ...
        if(!Objects.isNull(customersInfluencerMapModel)) {
            ...
            customersInfluencerMapModel.setInfluencer(influencer);
            SclCustomerModel influencer1 = customersInfluencerMapModel.getInfluencer();
                   ...
                   LOG.info("1: PointRequisition:: Stock allocation to influencer:::" );
					...
					if (null != pointRequisitionModel
                        ...
                        try {
                            ...
                            sclNotificationService.submitDealerNotification((B2BCustomerModel) pointRequisitionModel.getRequestRaisedTo(),body,sub, NotificationCategory.DELIVERY_WINDOW_NOT_MET,null);
                            sclNotificationService.submitDealerNotification((B2BCustomerModel) pointRequisitionModel.getRequestRaisedFor(),body,sub,NotificationCategory.DELIVERY_WINDOW_NOT_MET,null);
                        }
                        catch(Exception e) {
    ...
    @Override
    public SearchPageData<SclCustomerModel> getSavedDealerRetailer(SearchPageData searchPageData) {
        ...
        return pointRequisitionDao.getList(searchPageData,sclCustomerModel,site);

    }


    @Override
    public SearchPageData<SclCustomerModel> getList(String filter, SearchPageData searchPageData) {
        SclCustomerModel sclCustomerModel=(SclCustomerModel) userService.getCurrentUser();
        BaseSiteModel site = baseSiteService.getCurrentBaseSite();
        List<SubAreaMasterModel> list = new ArrayList<SubAreaMasterModel>();
        UserModel user = userService.getCurrentUser();
        list = territoryManagementDao.getTerritoriesForCustomer(sclCustomerModel);
        SearchPageData<SclCustomerModel> allCustomerForTerritories = pointRequisitionDao.getAllCustomerForTerritories(filter,searchPageData, list);
        List<SclCustomerModel> collect = allCustomerForTerritories.getResults().stream().filter(d -> (d.getGroups()).contains(userService.getUserGroupForUID(SclCoreConstants.CUSTOMER.DEALER_USER_GROUP_UID)) || (d.getGroups()).contains(userService.getUserGroupForUID(SclCoreConstants.CUSTOMER.RETAILER_USER_GROUP_UID))).collect(Collectors.toList());
