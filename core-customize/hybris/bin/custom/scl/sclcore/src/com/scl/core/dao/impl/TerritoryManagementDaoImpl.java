...
public class TerritoryManagementDaoImpl implements TerritoryManagementDao {
    ...
    public static final String FETCH_RETAILERS_FOR_DEALER = "SELECT {d.retailer} FROM {DealerRetailerMapping AS d JOIN SclCustomer AS c ON {d.retailer}={c.pk}} WHERE {d.dealer}=?sclCustomer AND {c.active}=?active ";
    public static final String RETAILER_IN_DESCENDING_ORDER = "AND {d.retailer} IS NOT NULL ORDER BY {d.lastUsed} DESC";
    private static final Logger LOG = Logger.getLogger(DefaultSclCustomerAccountService.class);
    ...
    static String CUSTOMER_FOR_USER_QUERY = "CustomerSubAreaMapping as m join SclCustomer as c on {m.sclCustomer}={c.pk} ";
...
//    				params.put(CustomerSubAreaMappingModel.ISOTHERBRAND, Boolean.FALSE);
    ...
    public List<SclUserModel> getSclUsersForSubArea(List<SubAreaMasterModel> subAreaMasters) {
        ...
        final String queryString = "SELECT distinct({u:sclUser}) FROM {UserSubAreaMapping as u join SclUser as su on {u:sclUser}={su:pk}} WHERE {u:subAreaMaster} in ( ?subAreas) and {su:active}=?active and {u:isActive}=?active ";
    ...
    //USEFULL
    @Override
    public List<SclCustomerModel> getRetailerListForDealer(SclCustomerModel sclCustomer) {
        final Map<String, Object> params = new HashMap<String, Object>();
        final StringBuilder builder = new StringBuilder("select DISTINCT({d.retailer}) from {DealerRetailerMapping as d join SclCustomer as c on {d.retailer}={c.pk}} WHERE {d.dealer}=?sclCustomer AND {c.active}=?active");
        boolean active = Boolean.TRUE;
        params.put("sclCustomer", sclCustomer);
        params.put("active", active);
    ...
    public List<SubAreaMasterModel> getTerritoriesForCustomer(UserModel customer) {
        ...
        final Map<String, Object> params = new HashMap<String, Object>();
        final StringBuilder builder = new StringBuilder("SELECT {subAreaMaster} FROM {CustomerSubAreaMapping} WHERE {sclCustomer} = ?customer AND {isActive} = ?active AND {brand} = ?brand and {sclCustomer} is not null ");
      ...
      //  params.put(CustomerSubAreaMappingModel.ISOTHERBRAND, Boolean.FALSE);
        ...
        query.setResultClassList(Arrays.asList(SubAreaMasterModel.class));
        query.addQueryParameters(params);
        LOG.info(String.format("getTerritoriesForCustomer query::%s",query));
        final SearchResult<SubAreaMasterModel> searchResult = flexibleSearchService.search(query);
        List<SubAreaMasterModel> result = searchResult.getResult();
        return result != null && !result.isEmpty() ? result : Collections.emptyList();
    }
    ...
    public List<SclCustomerModel> getSCLAndNonSCLCustomerForSubArea(List<SubAreaMasterModel> subAreaMaster) {
        final Map<String, Object> params = new HashMap<String, Object>();
        final StringBuilder builder = new StringBuilder("SELECT {cus.sclCustomer} FROM {CustomerSubAreaMapping as cus join SclCustomer as sc on {sc:pk}={cus:sclCustomer} } WHERE {cus.subAreaMaster} in (?subAreaMaster) AND {cus.isActive} = ?active AND {cus.brand} = ?brand  ");
        ...
        if(BooleanUtils.isTrue(Boolean.valueOf(enbleInfuencerSite))) {
            builder.append(" and {sc:counterType} not in (?counterList) ");
        ...
        query.setResultClassList(Collections.singletonList(SclCustomerModel.class));
        query.addQueryParameters(params);
        LOG.info(String.format("getSCLAndNonSCLCustomerForSubArea:: %s",query));
        final SearchResult<SclCustomerModel> searchResult = flexibleSearchService.search(query);
        List<SclCustomerModel> result = searchResult.getResult();
        return result != null && !result.isEmpty() ? result : Collections.emptyList();
    }
    ...
    @Override
    public List<SclCustomerModel> getDealerListForRetailer(SclCustomerModel currentUser, BaseSiteModel currentSite){
        final Map<String, Object> attr = new HashMap<>();
    final StringBuilder builder = new StringBuilder("select DISTINCT {d.dealer} from {DealerRetailerMapping as d join SclCustomer as c on {d.dealer} = {c.pk} } where {d.retailer}=?currentUser AND {c.active} = ?active ");
    boolean active = Boolean.TRUE;
        attr.put("currentUser", currentUser);
        attr.put("active", active);
       // attr.put("currentSite", currentSite);
        final FlexibleSearchQuery query = new FlexibleSearchQuery(builder.toString());
	     query.setResultClassList(Collections.singletonList(SclCustomerModel.class));
        query.getQueryParameters().putAll(attr);
       LOG.info(String.format("getDealerListForRetailer---::%s",query));
        List<SclCustomerModel> resultList = flexibleSearchService.<SclCustomerModel>search(query).getResult();
    ...
    public List<SubAreaMasterModel> getTalukaForUser(FilterTalukaData filterTalukaData) {
        ...
        if (currentUser instanceof SclUserModel) {
            if (currentUser.getUserType() != null) {
                if (currentUser.getUserType().getCode().equals("SO")  || currentUser.getUserType().getCode().equals("TSM") || currentUser.getUserType().getCode().equals("RH") ) {
                    builder.append("select distinct({s:pk}) from {UserSubAreaMapping as u join SubAreaMaster as s on {u:subAreaMaster}={s:pk}}" +
                ...
                /*else if (currentUser.getUserType().getCode().equals("TSM")) {
                    ...
                    builder.append("select distinct({s:pk}) from {TsoTalukaMapping as t join SubAreaMaster as s on {t:subAreaMaster}={s:pk}}" +
                            " where {t:tsoUser} = ?sclUser and  {t:isActive} = ?active");
                    params.put("sclUser", currentUser);
                    if (filterTalukaData != null && !ObjectUtils.isEmpty(filterTalukaData) && filterTalukaData.getDistrictCode() != null && !filterTalukaData.getDistrictCode().isEmpty()) {
                        builder.append(" and {s:district} like ?districtCode");
                        params.put("districtCode", filterTalukaData.getDistrictCode().toUpperCase() + "%");
                    }
                }
        ...
        query.addQueryParameters(params);
        LOG.info(String.format("fetching getTalukaForUser::%s",query));
        final SearchResult<SubAreaMasterModel> searchResult = flexibleSearchService.search(query);
    ...
    @Override
    public List<SubAreaMasterModel> getTalukasForUser(String state) {
        final Map<String, Object> params = new HashMap<String, Object>();
        SclUserModel currentUser = (SclUserModel) userService.getCurrentUser();
        final StringBuilder builder = new StringBuilder();
            ...
            if (currentUser.getUserType() != null) {
                if (currentUser.getUserType().getCode().equals("SO") || currentUser.getUserType().getCode().equals("TSM") || currentUser.getUserType().getCode().equals("RH")) {
                    builder.append("select {s:pk} from {UserSubAreaMapping as u join SubAreaMaster as s on {u:subAreaMaster}={s:pk}}" +
                            " where {u:sclUser} = ?sclUser and {u:brand} = ?brand and {u.state}=?state and {u:isActive} = ?active");
                    params.put("sclUser", currentUser);
                }
        ...
        query.setResultClassList(Collections.singletonList(SubAreaMasterModel.class));
        query.addQueryParameters(params);
        LOG.info("getTalukasForUser :: "+ query);
        final SearchResult<SubAreaMasterModel> searchResult = flexibleSearchService.search(query);
        if (searchResult.getResult() != null && !searchResult.getResult().isEmpty()) {
            return searchResult.getResult();
        } else {
    ...
    public List<SclCustomerModel> getCustomerForUser(RequestCustomerData requestCustomerData, List<SubAreaMasterModel> subAreaMasterList) {
        ...
        query.setResultClassList(Collections.singletonList(SclCustomerModel.class));
        query.addQueryParameters(params);
        LOG.info(String.format("getCustomerForUser query::%s",query));
        final SearchResult<SclCustomerModel> searchResult = flexibleSearchService.search(query);
        List<SclCustomerModel> result = searchResult.getResult();
...
//            params.put("isDealerFlag", Boolean.FALSE);
    ...
    public List<UserSubAreaMappingModel> getSOSubAreaMappingForUser(FilterTalukaData filterTalukaData) {
        {
            ...
            if (currentUser instanceof SclUserModel) {
                if (currentUser.getUserType().getCode().equals("SO") || currentUser.getUserType().getCode().equals("TSM") || currentUser.getUserType().getCode().equals("RH")) {
                    builder.append("select {u:pk} from {UserSubAreaMapping as u join SubAreaMaster as s on {u:subAreaMaster}={s:pk}}" +
                            " where {u:sclUser} = ?sclUser and {u:brand} = ?brand and {u:isActive} = ?active");
                    params.put("sclUser", currentUser);
                }
    ...
    public List<SclCustomerModel> getDealersForSP(RequestCustomerData requestCustomerData) {
        ...
        query.setResultClassList(Collections.singletonList(SclCustomerModel.class));
        query.addQueryParameters(params);
        LOG.info(String.format("getDealersForSp query ::%s",query));
        final SearchResult<SclCustomerModel> searchResult = flexibleSearchService.search(query);
        List<SclCustomerModel> result = searchResult.getResult();
        return result != null && !result.isEmpty() ? result : Collections.emptyList();
    }
    ...
    public List<SubAreaMasterModel> getTalukaForSP(FilterTalukaData filterTalukaData) {
        ...
        query.setResultClassList(Collections.singletonList(SubAreaMasterModel.class));
        query.addQueryParameters(params);
        LOG.info(String.format("getTalukaForSP query ::%s",query));
        final SearchResult<SubAreaMasterModel> searchResult = flexibleSearchService.search(query);
        List<SubAreaMasterModel> result = searchResult.getResult();
        return result != null && !result.isEmpty() ? result : Collections.emptyList();
    ...
    @Override
    public List<SclUserModel> getTSOforSubArea(SubAreaMasterModel subAreaMaster) {
        final Map<String, Object> params = new HashMap<String, Object>();
    ...
    @Override
    public SearchPageData<SclCustomerModel> fetchRetailersForDealer(SearchPageData searchPageData, SclCustomerModel sclCustomer, BaseSiteModel site, String searchKey) {
        final Map<String, Object> params = new HashMap<String, Object>();
        final StringBuilder builder = new StringBuilder(FETCH_RETAILERS_FOR_DEALER);
        boolean active = Boolean.TRUE;
        params.put("sclCustomer", sclCustomer);
        params.put("brand", site);
        params.put("active", active);
        appendFilterQuery(builder, params, null, Boolean.FALSE, searchKey, null, null);
        builder.append(RETAILER_IN_DESCENDING_ORDER);
        final PaginatedFlexibleSearchParameter parameter = new PaginatedFlexibleSearchParameter();
        parameter.setSearchPageData(searchPageData);
        final FlexibleSearchQuery query = new FlexibleSearchQuery(builder.toString());
        query.setResultClassList(Collections.singletonList(SclCustomerModel.class));
        query.getQueryParameters().putAll(params);
        parameter.setFlexibleSearchQuery(query);
        SearchPageData<SclCustomerModel> customers  = paginatedFlexibleSearchService.search(parameter);
        List<SclCustomerModel> customerList = (customers.getResults() != null) ? customers.getResults() : Collections.emptyList();
        if(CollectionUtils.isNotEmpty(customerList)){
            customers.setResults(customerList.stream().distinct().collect(Collectors.toList()));
        }
        return customers;
    }
    ...
    public List<SubAreaMasterModel> getTerritoryForCustWithAllBrands(SclCustomerModel customer) {
        final Map<String, Object> params = new HashMap<String, Object>();
        final StringBuilder builder = new StringBuilder("SELECT {subAreaMaster} FROM {CustomerSubAreaMapping} WHERE {sclCustomer} = ?customer AND {isActive} = ?active  and {sclCustomer} is not null ");
        params.put("customer", customer);
        params.put("active", Boolean.TRUE);
       // params.put("brand", baseSiteService.getCurrentBaseSite());
        //  params.put(CustomerSubAreaMappingModel.ISOTHERBRAND, Boolean.FALSE);
    ...
    public List<SclUserModel> getSOforSite(TerritoryMasterModel territoryMaster) {
        final Map<String, Object> params = new HashMap<String, Object>();
        final StringBuilder builder = new StringBuilder();
        builder.append("select {su:pk} from {TerritoryUserMapping as tu join TerritoryMaster as ts on {tu:territoryMaster}={ts:pk} join SclUser as su on {su:pk}={tu:sclUser}} " +
                " where {tu:brand} = ?brand and {tu:isActive} = ?active and {ts:pk}=?territoryMaster");
        ...
        query.addQueryParameters(params);
        final SearchResult<SclUserModel> searchResult = flexibleSearchService.search(query);
        LOG.info(String.format("getSOforSite query ::%s",query));
