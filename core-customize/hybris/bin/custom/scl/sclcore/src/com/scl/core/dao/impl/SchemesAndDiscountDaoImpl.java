...
//		}
	...
	public List<OrderModel> getGiftOrdersForDistrictAndTaluka(List<String> districtSubarea, String status, String key) {
		...
		final StringBuilder builder = new StringBuilder("SELECT {pk} FROM {Order AS o JOIN SclCustomer AS c ON {o.user}={c.pk} } WHERE {o.crmOrderType}=?Gift AND CONCAT({o.district},'_',{o.subArea}) IN (?districtSubArea)");
    ...
    public SearchPageData<OrderModel> getPaginatedGiftOrderList(SearchPageData searchPageData, String scheme, String searchFilter, List<String> status) {
		...
		if(userService.getCurrentUser().getGroups().contains(userService.getUserGroupForUID(SclCoreConstants.CUSTOMER.INFLUENCER_USER_GROUP_UID)))
		{
			if(searchFilter!=null){
				builder.append("select {o.pk} from {Order as o JOIN SclCustomer as u on {u:pk}={o:user} } where {o.crmOrderType}=?Gift and {o.user}=?currentuser ");
			}
			else{
			...
			if(searchFilter!=null){
				builder.append("select {o.pk} from {Order as o JOIN SclCustomer as u on {u:pk}={o:user} } where {o.crmOrderType}=?Gift");
			}
			else{
	...
	public SearchPageData<GiftShopModel> getPaginatedGiftShopSchemeList(SearchPageData searchPageData, String scheme, String searchFilter, List<String> status) {
		...
		StringBuilder builder = new StringBuilder();
		if(searchFilter!=null){
			builder.append("select {o.giftShop} from {Order as o JOIN SclCustomer as u on {u:pk}={o:user} } where {o.crmOrderType}=?Gift  ");
		}
		else{
