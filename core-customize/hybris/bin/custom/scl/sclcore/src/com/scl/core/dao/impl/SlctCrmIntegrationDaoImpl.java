...
public class SlctCrmIntegrationDaoImpl implements SlctCrmIntegrationDao {
	...
	public List<SclCustomerModel> getAllInfluencerDetails() {
		...
		final Map<String, Object> params = new HashMap<String, Object>();
		final StringBuilder builder = new StringBuilder("select {pk} from {SclCustomer} where {counterType} = ?counterType and {applicationDate} is not null").append(this.appendIntegrationSetting("ONBOARDED_INFLUENCER_DETAILS",params));
	...
	public List<List<Object>> getAllRetailerSalesInfo() {
		...
		final Map<String, Object> params = new HashMap<String, Object>();
		final StringBuilder builder = new StringBuilder("select {ord.code},{ord.erpOrderNumber},{deal.customerNo},{ret.uid}, {ret.customerNo} from {Order as ord JOIN SclCustomer as deal on {ord.user}={deal.pk} JOIN SclCustomer as ret on {ord.retailer}={ret.pk}} where {ret.synced} = ?status and {ord.erpOrderNumber} IS NOT NULL and {ord.retailer} IS NOT NULL");
...
//			GeographicalMasterModel transportationZone = geographicalRegionDao.fetchGeographicalMaster(destCityId);
			SclIncoTermMasterModel incoTermModel = defaultDestinationSourceMasterDao.findIncoTermByCode(incoTerm);
			...
			map.put(DestinationSourceMasterModel.TRANSPORTATIONZONE, destCityId);
			map.put(DestinationSourceMasterModel.PRODUCTCODE, productCode);
			map.put(DestinationSourceMasterModel.INCOTERMS, incoTermModel);
			
			final StringBuilder builder = new StringBuilder("select {pk} from {DestinationSourceMaster} where {deliveryMode}=?deliveryMode and {brand}=?brand and {orderType}=?orderType and {customerCategory}=?customerCategory and {productCode}=?productCode and {incoterms}=?incoterms and {transportationZone}=?transportationZone");
	...
	public SclCustomerModel getCustomerDetails(String uid) {
		final Map<String, Object> params = new HashMap<String, Object>();
		final StringBuilder builder = new StringBuilder("select {pk} from {SclCustomer} where {uid} = ?uid");
	...
	public SclCustomerModel getMitraMasterDetails(String customerID) {
		final Map<String, Object> params = new HashMap<String, Object>();
		final StringBuilder builder = new StringBuilder("select {pk} from {SclCustomer} where {customerID} = ?customerID");
	...
	public SclCustomerModel getCustomerByCustNo(String customerNo) {
		final Map<String, Object> params = new HashMap<String, Object>();
		final StringBuilder builder = new StringBuilder("select {pk} from {SclCustomer} where {customerNo} = ?customerNo");
	...
	public List<List<Object>> getSlctCrmCustomerDetails() {
		final Map<String, Object> params = new HashMap<String, Object>();
		final StringBuilder builder = new StringBuilder("select {cust.pk}, {sub.pk}, {counter.route} from {SclCustomer as cust join CustomerSubAreaMapping as sub on {cust.pk}={sub.sclCustomer} join CounterRouteMapping as counter on {cust.uid}={counter.counterCode}} where {cust.synced} = ?status and {sub.isActive} = ?active and {sub.brand} = ?brand");

		params.put("status", Boolean.FALSE);
		params.put("active", Boolean.TRUE);
		params.put("brand",(CMSSiteModel) baseSiteService.getBaseSiteForUID("scl"));

		final FlexibleSearchQuery query = new FlexibleSearchQuery(builder.toString());
		query.addQueryParameters(params);
		LOG.info(String.format("getSlctCrmCustomerDetails ::%s",query));
		query.setResultClassList(Arrays.asList(SclCustomerModel.class, CustomerSubAreaMappingModel.class, String.class));
		final SearchResult<List<Object>> searchResult = flexibleSearchService.search(query);
		return searchResult.getResult();
	}
	...
	@Override
	public CustomerSubAreaMappingModel getCustomerSubAreaMapByCustomer(SclCustomerModel sclCustomerModel) {
		final Map<String, Object> params = new HashMap<String, Object>();
		final StringBuilder builder = new StringBuilder("select {pk} from {CustomerSubAreaMapping} where {sclCustomer} = ?sclCustomerModel");
	...
	@Override
	public List<DealerPlannedMonthlySalesModel> getDealerPannedMonthlySalesDetails(List<SubAreaMasterModel> subAreaMasterList, SclUserModel salesOfficer, String monthName, String monthYear) {
		final Map<String, Object> params = new HashMap<String, Object>();
	...
	public List<DeliveryItemModel> getOrderLinesForEpod() {
		final Map<String, Object> params = new HashMap<String, Object>();
		final StringBuilder builder = new StringBuilder("select {pk} from {DeliveryItem} where ({epodCompletionSynced} = ?epodSynced and {epodInitiateDate} is not null) or ({epodFeedbackSynced} = ?feedbackSynced and {epodFeedback} is not null)");
		...
		query.addQueryParameters(params);
		final SearchResult<DeliveryItemModel> searchResult = flexibleSearchService.search(query);
		return searchResult.getResult();
	}
	...
	@Override
	public SclUserModel getDoByUidAndEmpCode(String uid) {
		final Map<String, Object> params = new HashMap<String, Object>();
		final StringBuilder builder = new StringBuilder("select {pk} from {SclUser} where {uid} = ?uid");
		params.put("uid", uid);
	...
	public UserSubAreaMappingModel getDoSubAreaMappingUid(String soUid, String brand, SubAreaMasterModel subAreaMaster, String state) {
		final Map<String, Object> params = new HashMap<String, Object>();
		final StringBuilder builder = new StringBuilder("select {usam:pk} from {UserSubAreaMapping as usam join SclUser as u on {usam:sclUser}={u:pk}} where {u:uid} = ?uid and {usam:brand} = ?brand and {usam:subAreaMaster} = ?subAreaMaster and {usam:state} = ?state");
	...
	public List<List<Object>> getCountofLeadsGenerated(Integer year, Integer month) {
		final Map<String, Object> params = new HashMap<String, Object>();
		final StringBuilder builder = new StringBuilder("select count(*),{a:district},{a:state} from {LeadMaster as l join Address as a on {a.pk}={l.address}} where ").append(SclDateUtility.getDateClauseQueryByMonthYear("l:creationTime", month, year, params));
	...
	public List<OrderModel> getModifiedOrdersOfYesterday(String date) {
		final Map<String, Object> params = new HashMap<String, Object>();
		final StringBuilder builder = new StringBuilder("select {pk} from {Order} where {modifiedtime} >= ?date and {versionId} is null");
	...
	public List<OrderEntryModel> getModifiedOrderEntries(String date) {
		final Map<String, Object> params = new HashMap<String, Object>();
		final StringBuilder builder = new StringBuilder("select {oe:pk} from {Order as o join OrderEntry as oe on {oe:order}={o:pk}} where {oe:modifiedtime} >= ?date and {o:versionId} is null");
	...
	public String appendIntegrationSetting(String integrationName, Map<String, Object> params) {
		...
		if(strategy!=null) {
			...
			if(strategy.equals("synced")) {
				...
				builder.append(SclDateUtility.getLastXDayQuery(columnName,params,integrationSettingModel.getLastXDays()));
			}
			else if (strategy.equals("syncedAndLastXDays")) {
				builder.append(syncedColumnName + "=0");
				builder.append(" and ").append(SclDateUtility.getLastXDayQuery(columnName,params,integrationSettingModel.getLastXDays()));
			}
		}
    ...
    public List<SclSiteMasterModel> getSiteMasterDetailsForSLCT() {
		final Map<String, Object> params = new HashMap<String, Object>();
		String builder = "select {pk} from {SclSiteMaster} Where ";
		...
		query.addQueryParameters(params);
		LOG.info("SclSiteMaster Integration Query used:"+query);
	...
	@Override
	public TsoTalukaMappingModel getTsoTalukaMappingByEmp(String soUid, String brand, SubAreaMasterModel subAreaMaster, String state) {
		final Map<String, Object> params = new HashMap<String, Object>();
		final StringBuilder builder = new StringBuilder("select {ttm:pk} from {TsoTalukaMapping as ttm join SclUser as u on {ttm:tsoUser}={u:pk}} where {u:uid} = ?uid and {ttm:brand} = ?brand and {ttm:subAreaMaster} = ?subAreaMaster and {ttm:state} = ?state");
		params.put("uid", soUid);
		params.put("brand", (CMSSiteModel) baseSiteService.getBaseSiteForUID(brand));
		params.put("subAreaMaster", subAreaMaster);
		params.put("state", state);
	...
	@Override
	public SalesSummaryModel getSalesSummaryModel(String customerNo, String sapProductCode, String state, String district, Integer month, Integer year) {
		final Map<String, Object> params = new HashMap<String, Object>();
		final StringBuilder builder = new StringBuilder("select {pk} from {SalesSummary} where {customerNo}=?customerNo and {sapProductCode}=?sapProductCode and {state}=?state and {district}=?district and {month}=?month and {year}=?year");

		params.put("customerNo", customerNo);
		params.put("sapProductCode", sapProductCode);
		params.put("state", state);
		params.put("district", district);
	...
	@Override
	public DeliveryItemModel findDeliveryItemByDiNumber(String diNumber) {
		final Map<String, Object> params = new HashMap<String, Object>();
		final StringBuilder builder = new StringBuilder("select {d:pk} from {DeliveryItem as d join OrderEntry as oe on {d:entry}={oe:pk} join Order as o on {oe:order}={o:pk}} where {d:diNumber} = ?diNumber");
	...
	@Override
	public List<MarketMappingDetailsModel> getMarketMappingbyIds(List<String> ids) {
		final Map<String, Object> params = new HashMap<String, Object>();
		final StringBuilder builder = new StringBuilder("select {pk} from {MarketMappingDetails} where {id} in (?ids)");
	...
	@Override
	public List<SclSiteMasterModel> getSiteTransactionDetailsForSLCT(List<String> siteStatusList) {
		final Map<String, Object> params = new HashMap<String, Object>();
		final StringBuilder builder = new StringBuilder("select distinct {scm:pk} from {SclSiteMaster as scm join SiteStatus as ss on ({scm:siteStatus}={ss:pk} or {scm.siteStatus} is null)} Where {ss:code} in (?siteStatus) and {scm:siteBagQtyMap} is not null").append(this.appendIntegrationSetting("SITE_TRANSACTION_DATA",params));
		params.put("siteStatus", siteStatusList);
 		LOG.info(String.format("Site Statuses passed in the Site Transaction integration query : %s",siteStatusList.toString()));
		final FlexibleSearchQuery query = new FlexibleSearchQuery(builder);
		query.setResultClassList(Collections.singletonList(SclSiteMasterModel.class));
		query.addQueryParameters(params);
		LOG.info("Site Transaction Integration Query used: "+query);
		final SearchResult<SclSiteMasterModel> searchResult = flexibleSearchService.search(query);
		return  searchResult.getResult();
	}
	...
	public List<SiteTransactionModel> getSiteConversionDetailsForSLCT() {
		final Map<String, Object> params = new HashMap<String, Object>();
		final StringBuilder builder = new StringBuilder("select distinct {st:pk} from {SiteTransaction as st join SclSiteMaster as scm on {st:site}={scm:pk}} where {st:siteConversionSynced} = ?siteConversionSynced");
		params.put("siteConversionSynced", Boolean.FALSE);
		final FlexibleSearchQuery query = new FlexibleSearchQuery(builder);
		query.setResultClassList(Collections.singletonList(SiteTransactionModel.class));
		query.addQueryParameters(params);
		LOG.info("Site Conversion Integration Query used: "+query);
		final SearchResult<SiteTransactionModel> searchResult = flexibleSearchService.search(query);
		return  searchResult.getResult();
	}

	@Override
	public List<SiteTransactionModel> getSiteTransactionsByIds(List<String> siteTransactionIds) {
		final Map<String, Object> params = new HashMap<String, Object>();
		final StringBuilder builder = new StringBuilder("select {pk} from {siteTransaction} where {id} in (?siteTransactionIds)");

		params.put("siteTransactionIds", siteTransactionIds);

		final FlexibleSearchQuery query = new FlexibleSearchQuery(builder.toString());
		query.setResultClassList(Collections.singletonList(SiteTransactionModel.class));
		query.addQueryParameters(params);
		LOG.info("Site Conversion Inbound API Query used: "+query);
		final SearchResult<SiteTransactionModel> searchResult = flexibleSearchService.search(query);
		if (searchResult.getResult() != null && !(searchResult.getResult().isEmpty()))
			return searchResult.getResult();
		else
  ...
  @Override
		public MasterStockAllocationModel getMasterStockAllocationModel(String invoiceNumber) {
		final Map<String, Object> params = new HashMap<String, Object>();
		final StringBuilder builder = new StringBuilder("select {pk} from {MasterStockAllocation} where {invoiceNumber}=?invoiceNumber");

		params.put("invoiceNumber", invoiceNumber);
//		params.put("invoiceLineNumber", invoiceLineNumber);

		final FlexibleSearchQuery query = new FlexibleSearchQuery(builder.toString());
		query.setResultClassList(Collections.singletonList(MasterStockAllocationModel.class));
		query.addQueryParameters(params);
		LOG.info(String.format("Master Stock ALlocation Integration Query:%s",query));
	...
	@Override
	public DealerRetailerMappingModel getRetForDealerShipto(String dealerCode, String shipToPartyCode) {
		final Map<String, Object> params = new HashMap<String, Object>();
		final StringBuilder builder = new StringBuilder("select {drm:pk} from {DealerRetailerMapping as drm join SclCustomer as cust on {drm:dealer}={cust:pk}} where {cust:uid} = ?dealerCode and {drm:partnerFunctionId} = ?shipToPartyCode");

		params.put("dealerCode", dealerCode);
		params.put("shipToPartyCode", shipToPartyCode);

		final FlexibleSearchQuery query = new FlexibleSearchQuery(builder.toString());
		query.setResultClassList(Collections.singletonList(DealerRetailerMappingModel.class));
		query.addQueryParameters(params);
		LOG.info(String.format("Dealer Retailer Mapping Query used for Master Stock ALlocation Integration:%s",query));
