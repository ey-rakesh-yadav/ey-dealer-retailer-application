...
public class EfficacyReportDaoImpl implements EfficacyReportDao{
	...
	public List<CounterVisitMasterModel> findCounterVisitForMonthYear(int month, int year, SubAreaMasterModel subArea) {
		...
		String searchQuery = "select {cv.pk} from {counterVisitMaster as cv join VisitMaster as v on {v.pk}={cv.visit}} where {v.subAreaMaster}=?subarea and {cv.endVisittime} is not null and {cv.counterType} in (?counterType) and "
				+ SclDateUtility.getDateClauseQueryByMonthYear("v.visitPlannedDate", month, year, params);
	...
	public Double findAfterSaleForDealer(Date visitDate, SclCustomerModel customer) {
		...
		String dateQuery = SclDateUtility.getDateRangeClauseQuery("oe.deliveredDate", visitLocalDate.toString(), endDate.toString(), params);
	...
	public Double findBeforeSaleForDealer(Date visitDate, SclCustomerModel customer) {
		...
		String dateQuery = SclDateUtility.getDateRangeClauseQuery("oe.deliveredDate", startDate.toString(), endDate.toString(), params);
	...
	@Override
	public EfficacyReportMasterModel getEfficacyReportForMonth(Integer month, Integer year, SubAreaMasterModel subArea, UserModel sclUser) {
		...
		String searchQuery = "select {e.pk} from {EfficacyReportMaster as e} where {e.month}=?month and {e.year}=?year and {e.subAreaMaster}=?subArea and {e.sclUser}=?sclUser";
		final FlexibleSearchQuery query = new FlexibleSearchQuery(searchQuery);
		query.addQueryParameters(params);
	...
	//New Territory Change
	@Override
	public List<VisitMasterModel> getAllVisitMasterForSubAreaAndSO(Date startDate, Date endDate, SubAreaMasterModel subarea, SclUserModel so) {
		final Map<String, Object> params = new HashMap<String, Object>();
	...
	@Override
	public Double getMonthlySalesForNewProduct(ProductModel product, List<SclCustomerModel> sclCustomerList, Date startDate, Date endDate) {
		final Map<String, Object> params = new HashMap<String, Object>();
		final StringBuilder builder = new StringBuilder("SELECT SUM({oe.quantityInMT}) FROM {OrderEntry AS oe JOIN Order AS o ON {oe.order}={o.pk} JOIN Product AS p ON {oe.product}={p.pk} } WHERE {oe.product}=?product AND {o.user} IN (?sclCustomerList) AND {oe.invoiceCreationDateAndTime} BETWEEN ?startDate AND ?endDate GROUP BY {p.code},{p.name}");
		params.put("product", product);
		params.put("sclCustomerList", sclCustomerList);
		params.put("startDate", startDate);
		params.put("endDate", endDate);
		final FlexibleSearchQuery query = new FlexibleSearchQuery(builder.toString());
		query.setResultClassList(Collections.singletonList(Double.class));
	...
	@Override
	public Double getMonthlySalesForDealer(SclCustomerModel dealer, Date startDate, Date endDate) {
		final Map<String, Object> params = new HashMap<String, Object>();
	...
	@Override
	public Double getMonthlySalesForRetailer(SclCustomerModel retailer, Date startDate, Date endDate) {
		final Map<String, Object> params = new HashMap<String, Object>();
	...
	@Override
	public List<SclCustomerModel> getObsoleteCountersList(SclUserModel sclUser, Date startDate, Date endDate) {
		final Map<String, Object> params = new HashMap<String, Object>();
		final StringBuilder builder = new StringBuilder("SELECT DISTINCT {cvm.sclCustomer} FROM {CounterVisitMaster AS cvm JOIN VisitMaster AS v ON {cvm.visit}={v.pk} } WHERE {v.user}=?sclUser AND {v.visitPlannedDate} >= ?startDate AND {v.visitPlannedDate} < ?endDate AND {cvm.networkType}=?dormant and {cvm.endVisitTime} IS NOT NULL and {v.status}=?completed ");
	...
	@Override
	public List<SclCustomerModel> getRevivedCountersList(List<SclCustomerModel> obsoleteCounters) {
		final Map<String, Object> params = new HashMap<String, Object>();
		final StringBuilder builder = new StringBuilder("SELECT DISTINCT {c.pk} FROM {SclCustomer AS c} WHERE {c.pk} IN (?obsoleteCounters) AND {c.networkType}=?active");
	...
	@Override
	public Double getSalesForCustomerList(List<SclCustomerModel> customerList, Date startDate, Date endDate) {
		final Map<String, Object> params = new HashMap<String, Object>();
	...
	@Override
	public Double getActualSalesForDealer(SclCustomerModel sclCustomer, BaseSiteModel baseSite, Date startDate, Date endDate) {
		final Map<String, Object> params = new HashMap<String, Object>();
