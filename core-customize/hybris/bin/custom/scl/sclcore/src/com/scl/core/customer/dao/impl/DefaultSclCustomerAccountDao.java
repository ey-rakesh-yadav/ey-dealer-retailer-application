...
import com.scl.core.jalo.DataConstraint;
import com.scl.core.job.AutoCancellingRequestRaisedByInfluencerJob;
...
/**
 * class for SCL Customer Related DAO
 */
public class DefaultSclCustomerAccountDao extends DefaultCustomerAccountDao implements SclCustomerAccountDao {

    private static final Logger LOG = Logger.getLogger(DefaultSclCustomerAccountDao.class);
    ...
    @Override
    public List<AddressModel> findSclAddressBookDeliveryEntriesForCustomer(final CustomerModel customerModel,
                                                                        final Collection<CountryModel> deliveryCountries)
    {
        ...
        if(!(customerModel.getGroups().contains(getUserService().getUserGroupForUID(SclCoreConstants.CUSTOMER.RETAILER_USER_GROUP_UID))))
        {
            LOG.info("inside findSclAddressBookDeliveryEntriesForCustomer incase of dealer");
