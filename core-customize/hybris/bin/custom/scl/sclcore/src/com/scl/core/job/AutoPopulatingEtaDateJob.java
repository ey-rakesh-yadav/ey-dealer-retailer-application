...
import com.scl.core.model.EtaTrackerModel;
import com.scl.core.model.ISOMasterModel;
import com.scl.core.model.MonthlySalesModel;
        ...
        for(EtaTrackerModel etaTrackerModel : etaTrackerList) {
            if(etaTrackerModel.getOrderType()!=null && !etaTrackerModel.getOrderType().isEmpty()) {
                if(etaTrackerModel.getOrderType().equals("DI")) {
                    List<DeliveryItemModel> deliveryItemModelList = getDeliveryItemsByDeliveryId(etaTrackerModel.getDeliveryId());
                    if(!deliveryItemModelList.isEmpty()) {
                        for(DeliveryItemModel deliveryItemModel : deliveryItemModelList) {
                            deliveryItemModel.setEtaDate(etaTrackerModel.getEtaDate());
...
//    }
    ...
    public List<DeliveryItemModel> getDeliveryItemsByDeliveryId(String deliveryId) {
        final Map<String, Object> params = new HashMap<String, Object>();
        final StringBuilder builder = new StringBuilder("select {pk} from {DeliveryItem} where {diNumber} = ?deliveryId");
