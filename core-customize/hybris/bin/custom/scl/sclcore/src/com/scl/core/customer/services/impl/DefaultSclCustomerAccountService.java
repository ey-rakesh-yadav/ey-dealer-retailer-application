...
import com.scl.core.event.UpdateContactNumberEvent;
...
public class DefaultSclCustomerAccountService extends DefaultCustomerAccountService implements SclCustomerAccountService {
    ...
    public boolean isExistingContactNumber(final String contactNumber){
        ...
        final List<PhoneContactInfoModel> phoneContactInfos = getSclCustomerAccountDao().findContactInfoByNumber(contactNumber);
    ...
    public SearchPageData<OrderEntryModel> fetchSOOrdersEntriesByCustomerAndStore(final CustomerModel user, final BaseStoreModel store, final String statuses, final SearchPageData searchPageData){
        ...
        return getSclCustomerAccountDao().findSOOrdersEntriesByCustomerAndStore(user,store,statusSet.toArray(new OrderStatus[statusSet.size()]),searchPageData);
    }
    ...
    public void setDefaultAddressEntry(final CustomerModel customerModel, final AddressModel addressModel){
        ...
        if(StringUtils.isBlank(addressModel.getPartnerFunctionId())){
            List<AddressModel> primaryAddressesFordealer = currentCustomerAddresses.stream().filter(addr -> addr.getIsPrimaryAddress() && StringUtils.isBlank(addr.getPartnerFunctionId())).collect(Collectors.toList());
            ...
            List<AddressModel> primaryAddressesForRetailer = currentCustomerAddresses.stream()
                    .filter(addr -> addr.getIsPrimaryAddress() && StringUtils.isNotBlank(addr.getPartnerFunctionId()) && addressModel.getPartnerFunctionId().equals(addr.getPartnerFunctionId()))
    ...
    @Override
    public void saveAddressEntryForRetailer(final SclCustomerModel retailer , final AddressModel addressModel , final boolean makeThisAddressDefault){
            ...
            clone.setPartnerFunctionId(StringUtils.EMPTY);
    ...
    @Override
    public List<AddressModel> getSclAddressBookDeliveryEntries(final CustomerModel customerModel)
    {
        validateParameterNotNull(customerModel, "Customer model cannot be null");
        return sclCustomerAccountDao.findSclAddressBookDeliveryEntriesForCustomer(customerModel,
