...
import com.scl.facades.data.marketvisit.VisitSummaryPlannedVsActualData;
import com.scl.facades.djp.data.AddNewSiteData;
import com.scl.facades.djp.data.CounterDetailsData;
import com.scl.facades.djp.data.ObjectiveData;
...
public class DJPVisitServiceImpl implements DJPVisitService {
    ...
    private static final String NOT_SCL_USER_MESSAGE = "SclCustomer is not present";
    ...
    public boolean submitMarketMappingDetails(CounterVisitMasterData data) {
        ...
        SclCustomerModel customer = counterVisitModel.getSclCustomer();
        ...
        if (data.getMarketMappingList() != null && !data.getMarketMappingList().isEmpty()) {
            data.getMarketMappingList().forEach(marketMappingData -> {
                ...
                if(CollectionUtils.isNotEmpty(userSubAreaList)) {
                    states.addAll(userSubAreaList.stream().map(data1 -> data1.getState()).distinct().collect(Collectors.toList()));
                }
    ...
    public boolean submitBrandInsightDetails(String counterVisitId, BrandingInsightData brandingInsightData) {
        ...
        if(brandingInsightData.getBrandingType()!=null){
            BrandingType enumerationValue = enumerationService.getEnumerationValue(BrandingType.class, brandingInsightData.getBrandingType());
            brandingInsightsDetailsModel.setBrandingType(enumerationValue);
        }
    ...
    private void populateSchmeDocuments(final SchemeDetailsModel schemeDetailsModel, List<MultipartFile> documents , final String brand) {
        ...
        for(MultipartFile file : documents){
            ...
            final String documentName = SclCoreConstants.SCHEME.SCHME_ATTR.concat(SclCoreConstants.UNDERSCORE_CHARACTER)
                    .concat(String.valueOf(currentTimeInMillis));
            CatalogUnawareMediaModel mediaModel = sclCustomerService.createMediaFromFile(null!= brand ?brand:StringUtils.EMPTY,documentName,file);
    ...
    public boolean submitLeadGeneration(LeadMasterData leadMasterData, String counterVisitId) {
        ...
        if(userService.getCurrentUser() instanceof SclCustomerModel) {
            if(!((SclCustomerModel) userService.getCurrentUser()).getCounterType().getCode().equalsIgnoreCase("SP")) {
                SclUserModel soForCustomer = territoryManagementService.getSOforCustomer((SclCustomerModel) userService.getCurrentUser());
       ...
       /* final AddressModel addressModel = modelService.create(AddressModel.class);
        ...
        try {
            ...
            if(userService.getCurrentUser() instanceof SclCustomerModel) {
                ...
                B2BCustomerModel so = territoryManagementService.getSOforCustomer((SclCustomerModel) leadModel.getCreatedBy());
                sclNotificationService.submitDealerNotification((B2BCustomerModel) leadModel.getCreatedBy(),body,sub, NotificationCategory.LEAD_GENERATED,suggestion);
                sclNotificationService.submitDealerNotification(so ,body,sub,NotificationCategory.LEAD_GENERATED,suggestion);
                ...
                for (SclUserModel tsoUser : tsoList) {
                    sclNotificationService.submitDealerNotification((B2BCustomerModel) tsoUser, body, sub, NotificationCategory.LEAD_GENERATED,suggestion);
                }
            }
            else if(userService.getCurrentUser() instanceof SclUserModel){
    ...
    public boolean submitTruckFleetDetails(DealerFleetListData dealerFleetListData, String counterVisitId) {
        ...
        if(dealerFleetListData.getDealerFleetList() != null && !dealerFleetListData.getDealerFleetList().isEmpty()) {
            for(DealerFleetData dealerFleetData: dealerFleetListData.getDealerFleetList())
            {
                ...
                TruckModelMasterModel truckModel = sclTruckDao.findTruckModelByModelNo(dealerFleetData.getTruckModel()!=null?dealerFleetData.getTruckModel().getTruckModel():null);
                ...
                if(counterVisitById!=null)
                    dealerFleetModel.setSclCustomer(counterVisitById.getSclCustomer());
    ...
    public boolean submitFlagDealer(String counterVisitId, boolean isFlagged, String remarkForFlag) {
        ...
        if(isFlagged) {
            try {
                ...
                counterVisitModel.setFlaggedBy(sclUser);
                counterVisitModel.setFlagTime(new Date());
                modelService.save(counterVisitModel);
                //setting in scl customer
                if (null != counterVisitModel.getSclCustomer()) {
                    SclCustomerModel sclCustomer = (SclCustomerModel) userService.getUserForUID(counterVisitModel.getSclCustomer().getUid());
                    sclCustomer.setIsDealerFlag(isFlagged);
                    sclCustomer.setRemarkForFlag(remarkForFlag);
                    sclCustomer.setFlaggedBy(sclUser);
                    sclCustomer.setFlagTime(new Date());
    ...
    public boolean submitUnflagDealer(String counterVisitId, boolean isUnFlagged, String remarkForUnflag) {
        ...
        if(isUnFlagged) {
            try {
                ...
                counterVisitModel.setUnFlagRequestRaisedBy(sclUser);
                ...
                if (null != counterVisitModel.getSclCustomer()) {
                    SclCustomerModel sclCustomer = (SclCustomerModel) userService.getUserForUID(counterVisitModel.getSclCustomer().getUid());
                    sclCustomer.setRemarkForUnflag(remarkForUnflag);
                    sclCustomer.setIsUnFlagRequestRaised(true);
                    sclCustomer.setUnFlagRequestRaisedBy(sclUser);
                    sclCustomer.setUnflagRequestTime(new Date());
                    sclCustomer.setUnFlagApprovalStatus(NetworkAdditionStatus.PENDING_FOR_APPROVAL_BY_TSM);
                    modelService.save(sclCustomer);
                }
    ...
    public boolean submitFeedbackAndComplaints(String counterVisitId, FeedbackAndComplaintsListData data) {
        ...
        if(null != data.getFeedbackAndComplaintsList() && !data.getFeedbackAndComplaintsList().isEmpty())
            for (FeedbackAndComplaintsData feedbackAndComplaintsData : data.getFeedbackAndComplaintsList()) {
                ...
                feedbackAndComplaintsModel.setFeedbackBy((SclCustomerModel) userService.getUserForUID(feedbackAndComplaintsData.getFeedbackByCode()));
    ...
    public DropdownListData getEnumTypes(String type) {
        ...
        if(null != type) {
            ...
            else if("DealerRequestStatus".equals(type)){
                enumType= DealerRequestStatus.class;
            }
            else if(SclCoreConstants.ENUM_TYPE.REASON_FOR_SITE_LOSS.equals(type)){
                enumType= ReasonForSiteLoss.class;
            }
            else if("BrandingRequestStatus".equals(type)){
                enumType= BrandingRequestStatus.class;
            }
            else if("brandingSiteType".equalsIgnoreCase(type))
            {
                enumType= BrandingSiteType.class;
            }
            else if ("pointOfSale".equalsIgnoreCase(type))
            {
                enumType= PointOfSale.class;
            }
            else if ("outdoors".equalsIgnoreCase(type))
            {
    ...
    public Collection<CompetitorProductModel> getCompetitorProducts(String brandId) {
        ...
        if(sclUser!=null && sclUser.getUserType()!=null && sclUser.getUserType().equals(SclUserType.TSO)){
            ...
            if (CollectionUtils.isNotEmpty(tsoTalukaMappingForTso)) {
                states.addAll(tsoTalukaMappingForTso.stream().map(TsoTalukaMappingModel::getState).distinct().collect(Collectors.toList()));
            }
        }else {
            List<UserSubAreaMappingModel> userSubAreaList = geographicalRegionDao.getUserSubAreaMappingForUser(sclUser);
            if (CollectionUtils.isNotEmpty(userSubAreaList)) {
    ...
    public List<TruckModelMasterModel> findAllTrucks() {
        List<TruckModelMasterModel> truckModelList = sclTruckDao.findAllTrucks();
    ...
    public void createAndSaveFinalizedCounterVisitPlan(final DJPFinalizedPlanWsDTO finalizedPlan){
        ...
        final SclUserModel sclUserModel = (SclUserModel) userService.getCurrentUser();
        DJPRouteScoreMasterModel djpRouteScoreMaster = null;
      //  if(sclUserModel.getUserType()!=null && sclUserModel.getUserType().equals(SclUserType.SO) && finalizedPlan.getRouteScoreId()!=null) {
        if(sclUserModel.getUserType()!=null && (sclUserModel.getUserType().equals(SclUserType.SO) || sclUserModel.getUserType().equals(SclUserType.TSM) || sclUserModel.getUserType().equals(SclUserType.RH)) && finalizedPlan.getRouteScoreId()!=null){
...
//      if(visitMasterDao.findByRouteAndDate(sclUserModel, djpRouteScoreMaster.getRun().getPlanDate())!=null) {
//          throw new IllegalArgumentException("Planned visit exists for selected planned date");
//      }

        VisitMasterModel visitMasterModel = createVisitMasterData(djpRouteScoreMaster , finalizedPlan.getObjectiveId(),sclUserModel, finalizedPlan.getSubAreaMasterId(), finalizedPlan.getPlanDate());
        if(sclUserModel.getUserType()!=null && (sclUserModel.getUserType().equals(SclUserType.SO) || sclUserModel.getUserType().equals(SclUserType.TSM) || sclUserModel.getUserType().equals(SclUserType.RH))  && finalizedPlan.getRoute()!=null && finalizedPlan.getRouteScoreId()==null) {
    ...
    @Override
    public List<SclCustomerModel> getFilteredCounters( DJPFinalizedPlanWsDTO plannedData){
        ...
        List<SclCustomerModel> allCounters = new ArrayList<SclCustomerModel>();
        ...
        if(routeMasterModel!=null) {
            List<SclCustomerModel> dealer = salesPerformanceDao.getCurrentNetworkCustomersForDJP("DEALER", territoryMasterModels, true, plannedData.getSubAreaMasterId(), routeMasterModel.getRouteId(),null);
            List<SclCustomerModel> retailer = salesPerformanceDao.getCurrentNetworkCustomersForDJP("RETAILER", territoryMasterModels, true, plannedData.getSubAreaMasterId(), routeMasterModel.getRouteId(),null);
            FilterTalukaData filterTalukaData=new FilterTalukaData();
            List<SclCustomerModel> nonsclCounters = salesPerformanceDao.getAllNonSclCustomers(territoryManagementService.getTaulkaForUser(filterTalukaData),plannedData.getSubAreaMasterId(),routeMasterModel.getRouteId(),null);
            allCounters.addAll(dealer);
            allCounters.addAll(retailer);
            allCounters.addAll(nonsclCounters);
        ...
        if(CollectionUtils.isNotEmpty(plannedData.getCounters())){
            ...
            return  allCounters.stream().filter(counter -> !(counterIdList.contains(counter.getUid()))).sorted(Comparator.comparing(SclCustomerModel::getCreationtime).reversed()).collect(Collectors.toList());
            //allCounters=allCounters.stream().sorted(Comparator.comparing(SclCustomerModel::getCreationtime).reversed()).collect(Collectors.toList());
            //return allCounters;
        }
        else{
            allCounters=allCounters.stream().sorted(Comparator.comparing(SclCustomerModel::getCreationtime).reversed()).collect(Collectors.toList());
    ...
    @Override
    public String getCustomerType(final SclCustomerModel sclCustomerModel) {
        String type = StringUtils.EMPTY;

        List<UserGroupModel> userGroupModelList=new ArrayList<>();
        if(Objects.nonNull(sclCustomerModel) && Objects.nonNull(sclCustomerModel.getGroups())){
            userGroupModelList = sclCustomerModel.getGroups().stream().filter(UserGroupModel.class ::isInstance).map(UserGroupModel.class::cast).collect(Collectors.toList());
        }
        if(CollectionUtils.isNotEmpty(userGroupModelList)){
            for(UserGroupModel userGroupModel : userGroupModelList){
                switch(userGroupModel.getUid()){
                    ...
                    case SclCoreConstants.CUSTOMER.DEALER_USER_GROUP_UID:
                        type = CounterType.DEALER.getCode();
                        break;

                    case SclCoreConstants.CUSTOMER.INFLUENCER_USER_GROUP_UID:
                        type = CounterType.INFLUENCER.getCode();
                        break;

                    case SclCoreConstants.CUSTOMER.SITE_USER_GROUP_UID:
                        type = CounterType.SITE.getCode();
                        break;

                    case SclCoreConstants.CUSTOMER.RETAILER_USER_GROUP_UID:
    ...
    public void createAndSaveSite(final AddNewSiteData newSiteData){
        ...
        SclCustomerModel siteToAdd = modelService.create(SclCustomerModel.class);
        ...
        final String defaultOtherUnitId = SclCoreConstants.B2B_UNIT.SCL_OTHER_UNIT_UID;
        populateDefaultUnitAndGroup(siteToAdd,defaultOtherUnitId,SclCoreConstants.CUSTOMER.SITE_USER_GROUP_UID);
    ...
    private void populateSiteTSOData(SclCustomerModel siteToAdd, final AddNewSiteData newSiteData) {
        //TSO attributes -->
    ...
    @Override
    public Set<RouteMasterModel> findAllRoutesForSO(SclUserModel sclUserModel){
        List<SclCustomerModel> associatedCustomers = new ArrayList<>(sclUserModel.getDealers());
        return associatedCustomers.stream().map(SclCustomerModel::getRoute).collect(Collectors.toSet());
    }

    private void populateServicingDealer(final SclCustomerModel siteToAdd,final  String servicingDealer) {

        try{
            SclCustomerModel sclCustomerModel = (SclCustomerModel)userService.getUserForUID(servicingDealer);
            if(null!=sclCustomerModel){
                siteToAdd.setSclCustomers(new ArrayList<>(List.of(sclCustomerModel)));
            }
        }
        catch (UnknownIdentifierException | ClassCastException ex){
            LOG.error("Error Occured while getting sclcustomer with uid: "+servicingDealer);
    ...
    private void populateDefaultUnitAndGroup(SclCustomerModel customer, String defaultUnitId, String userGroupUid) {
    ...
    private void populateSelectedRouteToSite(final SclCustomerModel siteToAdd, final String routeId){
    ...
    private List<SclCustomerModel> getAvailableCountersOnThisRouteScore(final String routeScoreId){
...
//        visitMasterModel.setApprovalStatus(ApprovalStatus.AUTO_APPROVED);
        visitMasterModel.setUser(sclUserModel);
...
//                    }
    ...
    public Integer getRetailerCountByTerritory(SclCustomerModel sclCustomerModel,Collection<TerritoryMasterModel> soTerritory) {
        return djpVisitDao.getRetailerCountByTerritory(sclCustomerModel,soTerritory);
    }
    ...
    public  CounterVisitMasterModel createCounterVisitMasterData(final DJPCounterWsDTO counter){
        ...
        if(StringUtils.isNotBlank(counter.getCustomerId())){
            try{
                counterVisitMasterModel.setSclCustomer((SclCustomerModel)userService.getUserForUID(counter.getCustomerId()));
            }
            catch (UnknownIdentifierException | ClassCastException ex){
                LOG.error("Error Occured while setting sclcustomer with uid: "+counter.getCustomerId());
                LOG.error("Exception is : "+ex.getMessage());
            }

        }
        if(null == counterVisitMasterModel.getSclCustomer() && null != djpCounterScoreMasterModel.getCustomer() ){
            counterVisitMasterModel.setSclCustomer(djpCounterScoreMasterModel.getCustomer());
        }
...
//        }
    ...
    public boolean startCounterVisit(String counterVisitId) {
        ...
        model.setStartVisitTime(new Date());
        model.setNetworkType(model.getSclCustomer().getNetworkType());
        model.setLastLiftingDate(model.getSclCustomer().getLastLiftingDate());
        model.setSynced(false);
        modelService.save(model);
        return true;
    }
        ...
        if(model!=null) {
            if(model.getStartVisitTime()!=null) {
                if(model.getEndVisitTime()==null) {
                    ...
                    model.setSynced(false);
                    modelService.save(model);
                    SclCustomerModel customer = model.getSclCustomer();
                    if(customer!=null) {
...
//              throw new IllegalArgumentException(String.format("Counter visit cannot be completed for not started counter visit for pk %s", counterVisitId));
    ...
    public Long getCountOfCounterNotVisited() {
        SclUserModel user = (SclUserModel)userService.getCurrentUser();
        Long count = 0L;
        AtomicReference<String> counterUid = new AtomicReference<>();
        try {
           //New Territory Change
           Collection<SclCustomerModel> counterList = territoryManagementService.getSCLAndNonSCLDealersRetailersForSubArea();
           //Collection<SclCustomerModel> counterList = territoryManagementService.getDealersRetailersForSubArea(territoryManagementService.getAllSubAreaForSO(user.getUid()));
           List<SclCustomerModel> filterCounterList = filterCustomerByDOTerritoryCode(List.copyOf(counterList));



           if (filterCounterList != null) {
               LOG.info(String.format("filterCounter list size ::%s",filterCounterList.size()));
               YearMonth currentMonth = YearMonth.now(ZoneId.systemDefault());
               count = filterCounterList.stream().filter(counter -> {
                   counterUid.set(counter.getUid());
                   if (counter.getLastVisitTime() != null) {
                       LocalDate date = counter.getLastVisitTime().toInstant()
                               .atZone(ZoneId.systemDefault())
                               .toLocalDate();
                       LOG.info(String.format("counter code::%s with name::%s and lastVisitTime ::%s for date::%s and currentMonth::%s",counter.getUid(),counter.getName(),counter.getLastVisitTime(),date,currentMonth));
                       return !currentMonth.equals(YearMonth.from(date));
                   }
                   LOG.info(String.format("Last visit time null counters ::%s with name::%s ",counter.getUid(),counter.getName()));
                   return true;
               }).count();
           }
           return count;
       }catch (Exception ex){
           LOG.error(String.format("Got exception in getCountOfCounterNotVisited ::%s with cause::%s ",ex.getMessage(),ex.getCause()));

           if(LOG.isDebugEnabled()){
             LOG.debug(String.format("exception stack trace for counter uid::%s not visited::%s",counterUid,ex.getStackTrace()));
           }
        }
        return count;
    }
    ...
    @Override
    public Collection<SclCustomerModel> getAdHocExistingCounters( DJPFinalizedPlanWsDTO plannedData){
        //New Territory Change
        Collection<SclCustomerModel> allCounters = new ArrayList<SclCustomerModel>();
        ...
        if(plannedData.getVisitId()!=null) {
            ...
            if(visitMaster!=null && visitMaster.getSubAreaMaster()!=null) {
                ...
                allCounters = territoryManagementService.getSCLAndNonSCLAllForSubArea(subAreas);
                LOG.info("All Counters getAdHocExistingCounters" + allCounters);
                if (CollectionUtils.isNotEmpty(allCounters)) {
                    allCounters = filterCustomerByDOTerritoryCode(allCounters.stream().collect(Collectors.toList()));
			}
            }
        }
        if(allCounters==null || allCounters.isEmpty()) {
            Collection<SclCustomerModel> sclDealers = salesPerformanceDao.getCurrentNetworkCustomers("DEALER", null, territoryMasterModels, null, true, false, false);
            Collection<SclCustomerModel> sclRetailers = salesPerformanceDao.getCurrentNetworkCustomers("RETAILER", null, territoryMasterModels, null, true, false, false);
            Collection<SclCustomerModel> nonSclCounters = salesPerformanceDao.getCurrentNetworkCustomers(null, null, territoryMasterModels, null, false, false, false);
            allCounters.addAll(sclDealers);
            allCounters.addAll(sclRetailers);
            allCounters.addAll(nonSclCounters);
            //allCounters = territoryManagementService.getSCLAndNonSCLAllForSO();
        }
        List<SclCustomerModel> allCountersFilteredList = new ArrayList<>();
        if(CollectionUtils.isNotEmpty(plannedData.getCounters())){
            final List<String> counterIdList = plannedData.getCounters().stream().map(DJPCounterWsDTO :: getCustomerId).collect(Collectors.toList());
    ...
    public boolean submitSiteVisitForm(String counterVisitId, SiteVisitFormData data) {
        CounterVisitMasterModel counterVisitModel = counterVisitDao.findCounterVisitById(counterVisitId);
        SclCustomerModel sclCustomer = counterVisitModel.getSclCustomer();

        if(data != null && sclCustomer != null) {
            sclCustomer.setCustomerType(SiteCustomerType.valueOf(data.getCustomerType()));
            sclCustomer.setContactPersonName(data.getContactPersonName());
            sclCustomer.setContactNumber(data.getContactNumber());
            sclCustomer.setIsShreeSite(data.getIsShreeCounter());
            sclCustomer.setIsBangurSite(data.getIsBangurCounter());
            sclCustomer.setIsRockstrongSite(data.getIsRockstrongCounter());
            BrandModel brandUnderUse = brandDao.findBrandById(data.getBrandUnderUse());
            sclCustomer.setBrandUnderUse(brandUnderUse);
            sclCustomer.setForPrice(data.getForPrice());
            ...
            sclCustomer.setPreferredBrandsList(preferredBrandMasterModels);
            sclCustomer.setAreaOfConstruction(data.getAreaOfConstruction());
            sclCustomer.setCurrentStageOfConstruction(CurrentStageOfSiteConstruction.valueOf(data.getCurrentStageOfConstruction()));
            sclCustomer.setBalancePotential(data.getBalancePotential());
            sclCustomer.setMonthlyConsumption(data.getMonthlyConsumption());
            sclCustomer.setCompletionPeriod(data.getCompletionPeriod());
            sclCustomer.setTargetOfConversion(TargetOfConversion.valueOf(data.getTargetOfConversion()));
            sclCustomer.setActivitiesPerformedAtSite(ActivitiesPerformedAtSite.valueOf(data.getActivitiesPerformedAtSite()));
            sclCustomer.setNextFollowUp(setNextFollowUpDate(data.getNextFollowUp()));
            if (data.getConversion().equalsIgnoreCase("yes")) {
                UserModel dealerName = userService.getUserForUID(data.getDealerNameOfConversion());
                sclCustomer.setDealerNameOfConversion((SclCustomerModel) dealerName);
                ...
                if(compProductsByCode!=null)
                    sclCustomer.setProductSelection(compProductsByCode);
                sclCustomer.setOrderReceived(data.getOrderReceived());
            }
            modelService.save(sclCustomer);
            return true;
        }
    ...
    public Collection<SclCustomerModel> getcounterNotVisitedList(int month, int year) {
        SclUserModel user = (SclUserModel)userService.getCurrentUser();
        Collection<SclCustomerModel> counterList = user.getDealers();
        Long count = 0L;
        if(counterList!=null) {
            YearMonth currentMonth = YearMonth.now(ZoneId.systemDefault());
            if(month!=0 && year!=0) {
    ...
    public boolean saveOrderRequisitionForTaggedSites(OrderRequistionMasterData orderRequisitionData, String counterVisitId, String siteCode) {
        ...
        if(siteCode!=null) {
            SclCustomerModel sclCustomer = (SclCustomerModel) userService.getUserForUID(siteCode);
            //sclCustomer.setTotalOrderGenerated(sum);
            modelService.save(sclCustomer);
            orderRequisition.setCounterVisit(counterVisitById);
            if(orderRequisitionData.getServingPartner()!=null)
                orderRequisition.setServingPartner((SclCustomerModel) userService.getUserForUID(orderRequisitionData.getServingPartner()));
            orderRequisition.setSiteCode(siteCode);
            orderRequisition.setTotalOrderGenerate(sum);
            orderRequisition.setSclCustomer(sclCustomer);
    ...
    public ErrorListWsDTO updateAdhocCounters(final UpdateCountersWsDTO adHocCountersWsDTO){
        ...
        if(CollectionUtils.isNotEmpty(adHocCountersWsDTO.getCounters())){
            for(DJPCounterWsDTO counter : adHocCountersWsDTO.getCounters()){
                if(SclCoreConstants.DJP.DELETE_UI_ACTION.equals(counter.getUiAction())){
                    removeCounterVisitFromList(counter.getCounterVisitId(),errorWsDTOS,true);
                }
                else if(SclCoreConstants.DJP.ADD_UI_ACTION.equals(counter.getUiAction())){
    ...
    @Override
    public CounterVisitAnalyticsWsDTO getCompletedVisitStatisticsDataForSO(final SclUserModel sclUserModel){
        ...
        List<VisitMasterModel> visitMasterModels = djpVisitDao.getCompletedPlannedVisitsBetweenDatesForSO(sclUserModel,formattedStartDate,formattedEndDate);
    ...
    private String getFormattedMonth(final LocalDate date) {
        DateTimeFormatter pattern = DateTimeFormatter.ofPattern(SclCoreConstants.DJP.STATISTICS_MONTH_PATTERN);
    ...
    private String getFormattedDate(final LocalDate date){
        if(null!= date){
            DateTimeFormatter pattern = DateTimeFormatter.ofPattern(SclCoreConstants.DJP.STATISTICS_DATE_PATTERN);
            return date.format(pattern);
        }
...
//        return visitsPlanned.stream().mapToLong(vp -> vp.getCounterVisits().size()).sum();
    ...
    public String getLastVisitDate(String counterVisitId) {
        ...
        if (null != counterVisitModel.getSclCustomer()) {
            sclCustomer = (SclCustomerModel) userService.getUserForUID(counterVisitModel.getSclCustomer().getUid());
            Date date = djpVisitDao.getLastVisitDate(sclCustomer);
            DateFormat dateFormat = new SimpleDateFormat(SclCoreConstants.CUSTOMER_ACCOUNT.DATE_FORMAT_1);
    ...
    public Integer getVisitCountMTD(String counterVisitId) {
        ...
        if (null != counterVisitModel.getSclCustomer()) {
            sclCustomer = counterVisitModel.getSclCustomer();
            mtdVisitCount = djpVisitDao.getVisitCountMTD(sclCustomer, month, year);
        }
        else{
            throw new ModelNotFoundException(NOT_SCL_USER_MESSAGE);
        }
    ...
    public DealerSummaryData getDealerSummary(String counterVisitId) {
        ...
        try {
            ...
            {
                if(Objects.nonNull(sclCustomer.getDealerCategory())){
                    data.setDealerCategory(sclCustomer.getDealerCategory().getCode());
                }
                int visits = djpVisitDao.getVisitCountMTD(sclCustomer, month1, year1);
                data.setVisits(visits);
                data.setPotential(sclCustomer.getCounterPotential());
                data.setLastVisitDate(sclCustomer.getLastVisitTime());
                data.setWholeSale(sclCustomer.getWholeSale());
                data.setCounterSale(sclCustomer.getRetailSale());
                List<List<Object>> currentMonthCounterShareInfo = djpVisitDao.getCounterSharesForDealerOrRetailer(counterVisitModel.getSclCustomer().getPk().toString(), month1, year1);
                try {
                ...
                List<List<Object>> lastMonthCounterShareInfo = djpVisitDao.getCounterSharesForDealerOrRetailer(counterVisitModel.getSclCustomer().getPk().toString(), month2, year2);
                try {
                ...
                if(sclCustomer.getCustomerNo()!=null) {
                    totalOutstandingAmount = djpVisitDao.getDealerOutstandingAmount(sclCustomer.getCustomerNo());
                ...
                if(sclCustomer.getCustomerNo()!=null) {
                    dailyAverageSales = collectionDao.getDailyAverageSalesForDealer(sclCustomer.getCustomerNo());
                }
               ...
               // data.setOutstandingAmount(Double.valueOf(amountFormatService.getFormattedValue(totalOutstandingAmount)));
                ...
                if(sclCustomer.getCustomerNo()!=null) {
                    creditLimit = djpVisitDao.getDealerCreditLimit(sclCustomer.getCustomerNo());
                }
                ...
                if(sclCustomer.getCustomerNo()!=null) {
                    setOutStandingBuckets(data,sclCustomer.getCustomerNo());

                    Date maxDate=djpVisitDao.getLastLiftingDateForDealer(sclCustomer.getCustomerNo(), brand);
                    if(maxDate != null) {
                        data.setLastLiftingDate(maxDate);

                        Double lastLiftingQuantity= djpVisitDao.getLastLiftingQuantityForDealer(sclCustomer.getCustomerNo(),brand,maxDate);
                    ...
                    Integer creditBreachedMTDCount = sclOrderCountDao.findCreditBreachCountMTD(sclCustomer);
                    data.setCreditBreachCount(creditBreachedMTDCount);
                    List<List<Double>> bucketList = djpVisitDao.getOutstandingBucketsForDealer(sclCustomer.getCustomerNo());
                ...
                //flag/unflag
                data.setIsDealerFlag(sclCustomer.getIsDealerFlag());
                data.setRemarkForFlag(sclCustomer.getRemarkForFlag());
                //data.setCurrentRemarkForFlag(sclCustomer.getCurrentRemarkForFlag());


                UserModel user = userService.getCurrentUser();
                userService.setCurrentUser(sclCustomer);
    ...
    public RetailerSummaryData getRetailerSummary(String code,List<String> subAreaList,List<String> districtList) {
        ...
        BaseSiteModel brand = baseSiteService.getCurrentBaseSite();
        String transactionType = SclCoreConstants.DJP.RETAILER_TRANSACTION_TYPE;


       /* SclUserModel sclUser = (SclUserModel) userService.getCurrentUser();

        List<SclCustomerModel> influencer = (List<SclCustomerModel>) sclUser.getDealers().stream().filter(r->(r.getGroups()).contains(userService.getUserGroupForUID(SclCoreConstants.CUSTOMER.INFLUENCER_USER_GROUP_UID))).collect(Collectors.toList());
        ...
        if (currentUser instanceof SclUserModel) {
            dealer = salesPerformanceService.getCustomersByLeadType("DEALER", null, null, null);
        ...
        SclCustomerModel sclCustomer = sclCustomerService.getSclCustomerForUid(code);
        try {
            if (sclCustomer != null) {
                String customerNo = sclCustomer.getCustomerNo();

                data.setName(sclCustomer.getName());
                data.setCustomerNo(customerNo);


                if (sclCustomer.getProfilePicture() != null && sclCustomer.getProfilePicture().getURL() != null && !sclCustomer.getProfilePicture().getURL().isEmpty()) {
                    data.setProfilePicUrl(sclCustomer.getProfilePicture().getURL());
                } else {
                    data.setProfilePicUrl("");
                }

               // if (customerNo != null) {
                    //Date maxDate=djpVisitDao.getLastLiftingDateForRetailerOrInfluencer(sclCustomer.getCustomerNo(), brand, transactionType);
                    //Date maxDate = djpVisitDao.getLastLiftingDateForRetailerFromOrderReq(sclCustomer, brand, transactionType, subAreaList, districtList)
                ...
                if(currentUser.getGroups().contains(userService.getUserGroupForUID(SclCoreConstants.CUSTOMER.DEALER_USER_GROUP_UID))) {
                    maxInvoicedDateAndQuantityForRetailer   = salesPerformanceDao.findMaxInvoicedDateAndQuantityForRetailer(sclCustomer, (SclCustomerModel) currentUser);
                }else{
                    maxInvoicedDateAndQuantityForRetailer = salesPerformanceDao.findMaxInvoicedDateAndQuantityForRetailer(sclCustomer, null);
                }
                if(maxInvoicedDateAndQuantityForRetailer!=null && maxInvoicedDateAndQuantityForRetailer.get("liftingQty")!=null
                        &&  maxInvoicedDateAndQuantityForRetailer.get("liftingDate")!=null){
                    double liftingQty = (double) maxInvoicedDateAndQuantityForRetailer.get("liftingQty");
                    String dateString= (String) maxInvoicedDateAndQuantityForRetailer.get("liftingDate");
                    String pattern="yyyy-MM-dd HH:mm";
                    DateTimeFormatter formatter = DateTimeFormatter.ofPattern(pattern);
                    try {
                        LocalDate date = LocalDate.parse(dateString, formatter);
                        Date lastLiftingDate = Date.from(date.atStartOfDay(ZoneId.systemDefault()).toInstant());
                        data.setLastLiftingDate(lastLiftingDate);
                        LOG.info(String.format("Last Lifting Date for Retailer:%s:::Date:%s",sclCustomer.getUid(),lastLiftingDate));
                    } catch (DateTimeParseException e) {
                        LOG.info(String.format("Parse Exception handled in cron job last lifting date Cause:%s :: Message ::%s",e.getCause(),e.getMessage()));
                    }
                    data.setLastLiftingQuantity(liftingQty);
                    LOG.info(String.format("Last Lifting qty for Retailer:%s:::Qty:%s",sclCustomer.getUid(),liftingQty));
                    LOG.info("setting Last Lifting date in 360:"+data.getLastLiftingDate());
                    LOG.info("setting Last Lifting qty in 360:"+data.getLastLiftingQuantity());
                }
                   /* if (maxDate != null) {
                        data.setLastLiftingDate(maxDate);

                        //Double lastLiftingQuantity= djpVisitDao.getLastLiftingQuantityForRetailerOrInfluencer(sclCustomer.getCustomerNo(),brand,maxDate , transactionType);
                        Double lastLiftingQuantity = djpVisitDao.getLastLiftingQuantityForRetailerFromOrderReq(sclCustomer, brand, maxDate, transactionType, subAreaList, districtList);
                ...
                if (sclCustomer.getLastVisitTime() != null) {
                    data.setLastVisitDate(sclCustomer.getLastVisitTime());
                }
                if (sclCustomer.getNetworkType() != null) {
                    data.setNetworkType(sclCustomer.getNetworkType());
                } else {
                    data.setNetworkType("");
                }

                if (sclCustomer.getCounterPotential() != null) {
                    if (currentUser instanceof SclUserModel) {
                        data.setPotential(sclCustomer.getCounterPotential());
                        ...
                        if((currentUser.getGroups().contains(userService.getUserGroupForUID(SclCoreConstants.CUSTOMER.RETAILER_USER_GROUP_UID)))) {
                            data.setPotential((sclCustomer.getCounterPotential() * 1000) / 50);
                        }else{
                            data.setPotential(sclCustomer.getCounterPotential());
                        }
                    }
                ...
                Integer djpVisits = djpVisitDao.getVisitCountMTD(sclCustomer, LocalDate.now().getMonthValue(), LocalDate.now().getYear());
                if (djpVisits != null && djpVisits > 0) {
                    data.setVisits(djpVisits);    //mtd
                } else {
                    data.setVisits(0);
                }
                if (sclCustomer.getWholeSale() != null) {
                    data.setWholeSale(sclCustomer.getWholeSale());
                } else {
                    data.setWholeSale(0.0);
                }
                if (sclCustomer.getRetailSale() != null) {
                    data.setCounterSale(sclCustomer.getRetailSale());
                } else {
                ...
                Double actualMonthSale=0.0,lastMonthSales=0.0,lastYearSameMonthSales=0.0,salesYtd=0.0;

                LocalDate firstDayOfLastMonth = LocalDate.now().minusMonths(1).with(TemporalAdjusters.firstDayOfMonth());
                LocalDate lastDayOfLastMonth = LocalDate.now().minusMonths(1).with(TemporalAdjusters.lastDayOfMonth()).plusDays(1);

                LocalDate firstDayOfLastYearMonth = LocalDate.now().minusYears(1).with(TemporalAdjusters.firstDayOfMonth());
                LocalDate lastDayOfLastYearMonth = LocalDate.now().minusYears(1).with(TemporalAdjusters.lastDayOfMonth()).plusDays(1);
                ...
                Map<String, Double> salesQuantityForRetailerByMonth=new HashMap<>();

                if(currentUser instanceof  SclUserModel) {
                    salesQuantityForRetailerByMonth = networkDao.getSalesQuantityForRetailerByMonth(sclCustomer, firstDayOfMonth.toString(), lastDayOfMonth.toString(), null, null);
                    actualMonthSale=salesQuantityForRetailerByMonth.get("quantityInMT");
                    salesQuantityForRetailerByMonth = networkDao.getSalesQuantityForRetailerByMonth(sclCustomer, firstDayOfLastMonth.toString(), lastDayOfLastMonth.toString(), null, null);
                    lastMonthSales=salesQuantityForRetailerByMonth.get("quantityInMT");
                    salesQuantityForRetailerByMonth = networkDao.getSalesQuantityForRetailerByMonth(sclCustomer, firstDayOfLastYearMonth.toString(), lastDayOfLastYearMonth.toString(), null, null);
                    lastYearSameMonthSales=salesQuantityForRetailerByMonth.get("quantityInMT");
                    salesQuantityForRetailerByMonth=networkDao.getSalesQuantityForRetailerByMonth(sclCustomer,currentFinancialYearDate.toString(), currentYearCurrentDate.toString(),null,null);
                    salesYtd=salesQuantityForRetailerByMonth.get("quantityInMT");
                }
                 else if((currentUser.getGroups().contains(userService.getUserGroupForUID(SclCoreConstants.CUSTOMER.DEALER_USER_GROUP_UID)))) {
                    salesQuantityForRetailerByMonth   = networkDao.getSalesQuantityForRetailerByMonth(sclCustomer, firstDayOfMonth.toString(), lastDayOfMonth.toString(),dealer, null);
                    actualMonthSale=salesQuantityForRetailerByMonth.get("quantityInMT");
                    salesQuantityForRetailerByMonth   = networkDao.getSalesQuantityForRetailerByMonth(sclCustomer, firstDayOfLastMonth.toString(), lastDayOfLastMonth.toString(), dealer, null);
                    lastMonthSales=salesQuantityForRetailerByMonth.get("quantityInMT");
                    salesQuantityForRetailerByMonth   = networkDao.getSalesQuantityForRetailerByMonth(sclCustomer, firstDayOfLastYearMonth.toString(), lastDayOfLastYearMonth.toString(),dealer, null);
                    lastYearSameMonthSales=salesQuantityForRetailerByMonth.get("quantityInMT");
                    salesQuantityForRetailerByMonth  =networkDao.getSalesQuantityForRetailerByMonth(sclCustomer,currentFinancialYearDate.toString(), currentYearCurrentDate.toString(), dealer,null);
                    salesYtd=salesQuantityForRetailerByMonth.get("quantityInMT");
                } else if((currentUser.getGroups().contains(userService.getUserGroupForUID(SclCoreConstants.CUSTOMER.RETAILER_USER_GROUP_UID)))) {
                    salesQuantityForRetailerByMonth = networkDao.getSalesQuantityForRetailerByMonth(sclCustomer, firstDayOfMonth.toString(), lastDayOfMonth.toString(), null, null);
                    actualMonthSale=salesQuantityForRetailerByMonth.get("quantityInBags");
                    salesQuantityForRetailerByMonth = networkDao.getSalesQuantityForRetailerByMonth(sclCustomer, firstDayOfLastMonth.toString(), lastDayOfLastMonth.toString(), null, null);
                    lastMonthSales=salesQuantityForRetailerByMonth.get("quantityInBags");
                    salesQuantityForRetailerByMonth = networkDao.getSalesQuantityForRetailerByMonth(sclCustomer, firstDayOfLastYearMonth.toString(), lastDayOfLastYearMonth.toString(), null, null);
                    lastYearSameMonthSales=salesQuantityForRetailerByMonth.get("quantityInBags");
                    salesQuantityForRetailerByMonth=networkDao.getSalesQuantityForRetailerByMonth(sclCustomer,currentFinancialYearDate.toString(), currentYearCurrentDate.toString(),null,null);
                    salesYtd=salesQuantityForRetailerByMonth.get("quantityInBags");
                }
                Double monthlySalesTarget=salesPerformanceService.getMonthlySalesTargetForRetailer(sclCustomer.getUid(),baseSiteService.getCurrentBaseSite(),null);

                if(monthlySalesTarget!=0.0) {
                    double askingRate = (monthlySalesTarget - actualMonthSale) / getRemainingDaysInMonth();
                    if (askingRate != 0.0) {
                        data.setAskingRate(String.valueOf(getTwoDeimalPlaceFormat(askingRate)));
                    }
                }else {
                    if(getNoOfDaysPassedInMonth()!=0) {
                        double askingRate = actualMonthSale / getNoOfDaysPassedInMonth();
                        data.setAskingRate(String.valueOf(getTwoDeimalPlaceFormat(askingRate)));
                    }else{
                        data.setAskingRate(df.format(0.0));
                    }
                }

                //deno - have to chk
                if(actualMonthSale!= 0.0 && lastYearSameMonthSales!=0.0) {
                    double growth = ((actualMonthSale - lastYearSameMonthSales) / lastYearSameMonthSales) * 100;
                    if (growth != 0.0) {
                        data.setGrowth(amountFormatService.getFormattedValue(growth));
                    }
                }else {
                    data.setGrowth(df.format(0.0));
                }
                ...
                data.setSalesYtd(salesYtd);
                int currentMonth=LocalDate.now().getMonthValue();
                int currentYear=LocalDate.now().getYear();
                data.setCurrentMonthLastYear(String.valueOf(Month.of(currentMonth)).concat("-").concat(String.valueOf(currentYear-1)));
                data.setCurrentMonthCounterShare(salesPerformanceService.getCounterShareForDealer(sclCustomer, LocalDate.now().getMonthValue(), LocalDate.now().getYear()));//
                //data.setCurrentMonthCounterShare(getCounterShare(sclCustomer.getPk().toString(), LocalDate.now().getMonthValue(), LocalDate.now().getYear()));
                data.setLastMonthCurrentYear(String.valueOf(Month.of(currentMonth-1)).concat("-").concat(String.valueOf(currentYear)));
                //data.setLastMonthCounterShare(getCounterShare(sclCustomer.getPk().toString(),LocalDate.now().minusMonths(1).getMonthValue(), LocalDate.now().minusMonths(1).getYear()));
                LocalDate lastMonthValue = LocalDate.now().minusMonths(1);
                data.setLastMonthCounterShare(salesPerformanceService.getCounterShareForDealer(sclCustomer,lastMonthValue.getMonthValue(), lastMonthValue.getYear()));//
                ...
                if(currentUser instanceof  SclUserModel) {
                    dataList = getLastSixMonthSalesForRetailer(dataList, sclCustomer, null);
                }
                else if((currentUser.getGroups().contains(userService.getUserGroupForUID(SclCoreConstants.CUSTOMER.DEALER_USER_GROUP_UID)))) {
                    dataList = getLastSixMonthSalesForRetailer(dataList, sclCustomer, dealer);
                }
                ...
                if (dataList != null && !dataList.isEmpty()) {
                    for (MonthlySalesData monthlySalesData : dataList) {
                        LOG.info("Current month iteration:" + monthlySalesData.getActualSales());
                    }
                }else{
                    LOG.info("data list is empty");
                }

                double  totalTarget=0.0, behindTarget=0.0, aheadTarget=0.0;
                double achievementPercentage=0.0;

                 totalTarget = salesPerformanceService.getMonthlySalesTargetForRetailer(currentUser.getUid(), baseSiteService.getCurrentBaseSite(),null);
                if(actualMonthSale!=0.0 && totalTarget!=0.0)
                    achievementPercentage=(actualMonthSale/totalTarget)*100;
                ...
                salesListData.setTarget(totalTarget);
                if(achievementPercentage < 100)
                {
                    behindTarget=totalTarget-actualMonthSale;
                    salesListData.setBehindTotalTarget(behindTarget!=0.0 ? behindTarget :0.0);
                }
                else if(achievementPercentage > 100)
                {
                    aheadTarget=actualMonthSale-totalTarget;
                ...
                data.setSalesData(salesListData);
                  ...
                  /*  if(sclCustomer.getLastLiftingDate()!=null){
                        data.setLastLiftingDate(sclCustomer.getLastLiftingDate());
                        LOG.info("setting Last Lifting date:"+sclCustomer.getLastLiftingDate());
                    }
                    if(sclCustomer.getLastLiftingQuantity()!=null){
                        data.setLastLiftingQuantity(sclCustomer.getLastLiftingQuantity());
                        LOG.info("setting Last Lifting qty:"+sclCustomer.getLastLiftingQuantity());
                    }*/
            //}
        }
    ...
    public InfluencerSummaryData getInfluencerSummary(String counterVisitId,List<String> subAreaList,List<String> districtList) {
        ...
        try {
            ...
            SclCustomerModel sclCustomer = counterVisitModel.getSclCustomer();
            if(Objects.nonNull(sclCustomer)) {
                int visits = djpVisitDao.getVisitCountMTD(sclCustomer, month, year);
                data.setVisits(visits);
                data.setPotential(sclCustomer.getCounterPotential());
                data.setLastVisitDate(sclCustomer.getLastVisitTime());

                //Date maxDate=djpVisitDao.getLastLiftingDateForRetailerOrInfluencer(sclCustomer.getNirmanMitraCode(), brand, transactionType);
                Date maxDate=djpVisitDao.getLastLiftingDateForInfluencerFromPointReq(sclCustomer, brand, transactionType,subAreaList,districtList);
                if(maxDate != null) {
                    data.setLastLiftingDate(maxDate);

                    // Double lastLiftingQuantity= djpVisitDao.getLastLiftingQuantityForRetailerOrInfluencer(sclCustomer.getNirmanMitraCode(),brand,maxDate , transactionType);
                    Double lastLiftingQuantity= djpVisitDao.getLastLiftingQuantityForInfluencerFromPointReq(sclCustomer,brand,maxDate , transactionType,subAreaList,districtList);
                    data.setLastLiftingQuantity(lastLiftingQuantity);
    ...
    public CounterVisitMasterModel getCounterVisitMasterForLastVisitDate(String counterVisitId) {
        ...
        if(counterVisitModel!=null) {
            if (null != counterVisitModel.getSclCustomer()) {
                return counterVisitModel.getSclCustomer().getLastCounterVisit();
            }
            else{
                throw new ModelNotFoundException("SclCustomer not found");
            }
        }
        else
        {
    ...
    public List<DealersFleetDetailsModel> getDealerFleetDetails(String counterVisitId) {
        ...
        if(counterVisitModel!=null)
        {
            List<DealersFleetDetailsModel> list = counterVisitModel.getSclCustomer()!=null ? (List<DealersFleetDetailsModel>) counterVisitModel.getSclCustomer().getDealerFleetDetails() : Collections.emptyList();
            return list;
        }
    ...
    @Override
    public String calculatePlanComplianceForSODJP(SclUserModel sclUserModel , final Date planStartDate, final Date planEndDate){

        BigDecimal recommendedCompletedVisitCount = BigDecimal.ZERO;
        BigDecimal recommendedPlannedVisitCount = BigDecimal.ZERO;

        List<VisitMasterModel> completedVisits = djpVisitDao.getCompletedPlannedVisitsBetweenDatesForSO(sclUserModel,planStartDate,planEndDate);
        if(CollectionUtils.isNotEmpty(completedVisits)){
            for(VisitMasterModel visitMasterModel : completedVisits){
                if(visitMasterModel.getRouteScore()!=null) {
                    ...
                    List<SclCustomerModel> recommendedCustomer = counterScoreList.stream().filter(each->each.getObjective()!=null && each.getObjective().equals(visitMasterModel.getObjective())).map(each -> each.getCustomer()).collect(Collectors.toList());
                    ...
                    long countOfCompleted = counterVisitList.stream().filter(each->each.getEndVisitTime()!=null && recommendedCustomer!=null && recommendedCustomer.contains(each.getSclCustomer())).map(each -> each.getSclCustomer()).count();
    ...
    @Override
    public SclUserModel getCurrentSalesOfficer(){
        SclUserModel sclUserModel = null;
        try{
            sclUserModel = (SclUserModel) userService.getCurrentUser();
        }
    ...
    public List<B2BCustomerModel> getInfluencerDetails(String counterVisitId) {
        CounterVisitMasterModel counterVisitModel = getCounterVisitMasterForLastVisitDate(counterVisitId);
        List<B2BCustomerModel> siteList =  (List<B2BCustomerModel>) counterVisitModel.getSclCustomer().getTaggedPartners().stream().filter(o->o.getGroups().contains(userService.getUserGroupForUID(SclCoreConstants.CUSTOMER.SITE_USER_GROUP_UID))).collect(Collectors.toList());
        return siteList;
    }
    ...
    public boolean updateUnFlagRequestApprovalByTSM(UnFlagRequestApprovalData unFlagRequestApprovalData) {
        ...
        if (unFlagRequestApprovalData != null) {
            if (unFlagRequestApprovalData.getApprovalStatus().equals(Boolean.TRUE) &&
                ...
                try{
                    SclCustomerModel sclCustomerModel = (SclCustomerModel) userService.getUserForUID(unFlagRequestApprovalData.getDealerCode());
                    if(sclCustomerModel!=null) {
                        if(sclCustomerModel.getIsDealerFlag()) {
                            sclCustomerModel.setIsUnFlagRequestRaised(false);
                            sclCustomerModel.setIsDealerFlag(false);
                            sclCustomerModel.setUnflagTime(new Date());
                            sclCustomerModel.setUnFlaggedBy(currentUser);
                            sclCustomerModel.setUnFlagApprovalStatus(NetworkAdditionStatus.APPROVED_BY_TSM);
    ...
    public Double getTotalOrderGenerated(String siteCode, String counterVisitId) {
        CounterVisitMasterModel counterVisit=counterVisitDao.findCounterVisitById(counterVisitId);
        SclCustomerModel sclCustomer= (SclCustomerModel) userService.getUserForUID(siteCode);
        return djpVisitDao.getTotalOrderGenerated(sclCustomer,counterVisit);
    }
    ...
    public SalesHistoryData getSalesHistoryForDealer(String counterVisitId) {
        CounterVisitMasterModel counterVisitModel = counterVisitDao.findCounterVisitById(counterVisitId);
        String customerNo = counterVisitModel.getSclCustomer().getCustomerNo();
        String customerCode=counterVisitModel.getSclCustomer().getUid();

      //  BaseSiteModel currentBaseSite = baseSiteService.getCurrentBaseSite();
       // SclCustomerModel customer = counterVisitModel.getSclCustomer();
        B2BCustomerModel b2bcustomer= counterVisitModel.getSclCustomer();
       ...
       // LocalDate lastYearSameMonth = date.minusYears(1);
        ...
        double lastMonthSales= sclSalesSummaryService.getLastMonthSales(b2bcustomer, null);
       // double lastMonthSales = salesPerformanceDao.getActualTargetSalesForSelectedMonthAndYearForDealer(customer, currentBaseSite, lastMonth.getMonthValue(), lastMonth.getYear(),null);
        double secondLastMonthSales = sclSalesSummaryService.getSecondLastMonthSales(b2bcustomer);
       // double secondLastMonthSales = salesPerformanceDao.getActualTargetSalesForSelectedMonthAndYearForDealer(customer, currentBaseSite, lastTolastMonth.getMonthValue(), lastTolastMonth.getYear(),null);
        ...
        data.setLastMonthSales(lastMonthSales);
        lastYearCurrentMonth = sclSalesSummaryService.getLastYearCurrentMonthSales(b2bcustomer);
        ...
        salesMtd = sclSalesSummaryService.getCurrentMonthSales(b2bcustomer, Collections.emptyList());
       // salesMtd = salesPerformanceDao.getActualTargetSalesForSelectedMonthAndYearForDealer(customer, currentBaseSite, date.getMonthValue(), date.getYear(), null);
        ...
        double actualMonthSale = sclSalesSummaryService.getCurrentMonthSales(b2bcustomer, Collections.emptyList());
       // double actualMonthSale = salesPerformanceDao.getActualTargetSalesForSelectedMonthAndYearForDealer(customer, currentBaseSite, date.getMonthValue(), date.getYear(), null);
        ...
        if(customerNo!=null) {
            ...
            monthlySalesTarget = salesPerformanceService.getMonthlySalesTargetForDealer(counterVisitModel.getSclCustomer(),baseSiteService.getCurrentBaseSite(),null);
        }
        ...
        data.setWholeSale(((SclCustomerModel) b2bcustomer).getWholeSale());
        data.setCounterSale(((SclCustomerModel) b2bcustomer).getRetailSale());
    ...
    private void populateAdHocVisitsData(MarketVisitDetailsData marketVisitDetailsData, VisitMasterModel visitMasterModel) {
        ...
        if(CollectionUtils.isNotEmpty(adHocCountersVisitedList)){
            for(CounterVisitMasterModel counterVisitMasterModel: adHocCountersVisitedList ){
                ...
                adHocCounterVisitedData.setCustomerName(null!= counterVisitMasterModel.getSclCustomer()?counterVisitMasterModel.getSclCustomer().getName():StringUtils.EMPTY);
                adHocCounterVisitedData.setCustomerCode(null!= counterVisitMasterModel.getSclCustomer()?counterVisitMasterModel.getSclCustomer().getUid():StringUtils.EMPTY);
                adHocCounterVisitedData.setCustomerType(getCustomerType(counterVisitMasterModel.getSclCustomer()));
    ...
    private void populateCountersNotVisitedData(MarketVisitDetailsData marketVisitDetailsData, VisitMasterModel visitMasterModel) {
        ...
        if(CollectionUtils.isNotEmpty(countersNotVisited)){
            for(CounterVisitMasterModel counterVisitMasterModel: countersNotVisited ){
                ...
                counterNotVisitedData.setCustomerName(null!= counterVisitMasterModel.getSclCustomer()?counterVisitMasterModel.getSclCustomer().getName():StringUtils.EMPTY);
                counterNotVisitedData.setCustomerCode(null!= counterVisitMasterModel.getSclCustomer()?counterVisitMasterModel.getSclCustomer().getUid():StringUtils.EMPTY);
                counterNotVisitedData.setCustomerType(getCustomerType(counterVisitMasterModel.getSclCustomer()));
    ...
    public MarketVisitDeviationData getCounterDeviationData(VisitMasterModel visitMasterModel, boolean journeyDetails) {
        ...
        List<SclCustomerModel> recommendedCounters = recommendedList.stream().map(DJPCounterScoreMasterModel::getCustomer).collect(Collectors.toList());

        List<SclCustomerModel> plannedCustomer = visitMasterModel.getCounterVisits().stream().map(CounterVisitMasterModel::getSclCustomer).collect(Collectors.toList());

        List<SclCustomerModel> deviationCounters = recommendedCounters.stream().filter(counter -> !plannedCustomer.contains(counter)).collect(Collectors.toList());
        ...
        if(journeyDetails || CollectionUtils.isNotEmpty(deviationCounters)){
            ...
            counterDeviationData.setSuggested(recommendedCounters.stream().filter(data->data!=null).map(SclCustomerModel::getName).collect(Collectors.toList()));
            List<String> selectedList = new ArrayList<String>();
            for(SclCustomerModel customer: plannedCustomer) {
    ...
    @Override
    public void createCounterRouteMapping(String state, String district, String taluka, BaseSiteModel brand, String counterCode,SclUserModel employee, String routeId, String routeName) {
    ...
    public List<MonthlySalesData> getLastSixMonthSalesForDealer(String counterVisitId) {
        ...
        CounterVisitMasterModel counterVisitModel = counterVisitDao.findCounterVisitById(counterVisitId);
        if (Objects.nonNull(counterVisitModel) && Objects.nonNull(counterVisitModel.getSclCustomer())) {
            SclCustomerModel customer = counterVisitModel.getSclCustomer();
            dataList=getMonthlySalesForDealer(dataList, customer);
        }
    ...
    @Override
    public List<MonthlySalesData> getMonthlySalesForDealer(List<MonthlySalesData> dataList, SclCustomerModel customer) {
        dataList=new ArrayList<>();

        List<B2BCustomerModel> customerModels=new ArrayList<>();
        customerModels.add(customer);
        LocalDate currentMonth = LocalDate.now();
        for (int i = 1; i <= 6; i++) {
            MonthlySalesData data = new MonthlySalesData();
            Double currentMonthSale = sclSalesSummaryService.getSalesByMonth(customer, currentMonth.getMonthValue(),currentMonth.getYear(),Collections.EMPTY_LIST);
            LocalDate lastMonth = currentMonth.minusMonths(1);
            Double lastMonthSale= sclSalesSummaryService.getSalesByMonth(customer, lastMonth.getMonthValue(),lastMonth.getYear(),Collections.EMPTY_LIST);
            data.setActualSales(getTwoDeimalPlaceFormat(currentMonthSale));
            //data.setActualSales(currentMonthSale);
            LOG.info("Inside sale:"+currentMonthSale);
            LOG.info("current month sale with rounded value:"+getTwoDeimalPlaceFormat(currentMonthSale));
            LOG.info("Last month sale with rounded value:"+getTwoDeimalPlaceFormat(lastMonthSale));
            //double growth = currentMonthSale - lastMonthSale;
            double growth = getTwoDeimalPlaceFormat(currentMonthSale)- getTwoDeimalPlaceFormat(lastMonthSale);
            //data.setGrowth(growth);
            data.setGrowth(getTwoDeimalPlaceFormat(growth));
            ...
            Double targetSale=salesPerformanceService.getMonthlySalesTargetForDealerList(customerModels,baseSiteService.getCurrentBaseSite(),currentMonth.getMonthValue(),currentMonth.getYear());
            //Double targetSale = salesPlanningDao.getDealerSalesAnnualTarget(customer.getUid(),monthYear);
            LOG.info("Inside target:"+targetSale);
           // data.setTargetSales(targetSale);
            data.setTargetSales(getTwoDeimalPlaceFormat(targetSale));
            if (targetSale > 0) {
                data.setPercentage(getTwoDeimalPlaceFormat((currentMonthSale / targetSale) * 100));
            }else {
            ...
            dataList.add(data);
            LOG.info(String.format("Inside dataList::%s for Monthaly sale data::%s",dataList,data));
            currentMonth = lastMonth;
        }
        LOG.info(String.format("Final data list ::%s",dataList));
        return dataList;
    }
    ...
    public MarketVisitDetailsData getJounreyDetailsData(final String visitId) {
        ...
        if (null != visitMasterModel.getCounterVisits() && !visitMasterModel.getCounterVisits().isEmpty())
        {
          ...
          /*  Date startDate1 = new Date();
            ...
            Date journeyStartDate = new Date();
            try {
                journeyStartDate = formatter.parse(marketVisitDetailsData.getDateOfJourney().toString());

            } catch (Exception e) {
                e.printStackTrace();
            }

            LocalDate date1=journeyStartDate.toInstant().atZone(ZoneId.systemDefault()).toLocalDate();
            LocalDate date2=date1.plusDays(1);

            LocalDate date3=date1.plusDays(1);
            LocalDate date4=date1.plusDays(1);

            LocalDate last3DaysStart=date1.minusDays(3);
            LocalDate last3DaysEnd=date1.minusDays(1);

            ZoneId defaultZoneId = ZoneId.systemDefault();
            Date orderStart= Date.from(date1.atStartOfDay(defaultZoneId).toInstant());
            Date orderend= Date.from(date2.atStartOfDay(defaultZoneId).toInstant());

            Date last3DaysStartDate= Date.from(last3DaysStart.atStartOfDay(defaultZoneId).toInstant());
            Date last3DaysEndDate= Date.from(last3DaysEnd.atStartOfDay(defaultZoneId).toInstant());

            Date orderStart1= Date.from(date3.atStartOfDay(defaultZoneId).toInstant());
            Date orderend1= Date.from(date4.atStartOfDay(defaultZoneId).toInstant());
            ...
            for(CounterVisitMasterModel counterVisit : visitMasterModel.getCounterVisits())
            {
                ...
                if(counterVisit.getSclCustomer()!=null) {
                    data.setCounterName(counterVisit.getSclCustomer().getName());
                    data.setCounterCode(counterVisit.getSclCustomer().getUid());
                    data.setCustomerNo(counterVisit.getSclCustomer().getCustomerNo());
                }
                data.setOrderBooked(djpVisitDao.getOrderCapturedForCounter(counterVisit.getSclCustomer(),orderStart,orderend));
                ...
                data.setPotentialBefore(djpVisitDao.getPreviousCounterPotentialForCounter(orderStart));
                ...
                //last 3 days
                Double last3DaysSales = djpVisitDao.getOrderCapturedForCounter(counterVisit.getSclCustomer(),last3DaysStartDate,last3DaysEndDate);
                //doubt
            /*    if(last3DaysSales!=0.0)
                    ...
                    data.setGrowth(0.0);
*/
                if(data.getOrderCaptured()!=0.0)
                    data.setGrowth(((todaysSales-data.getOrderCaptured())/data.getOrderCaptured())*100);
                else
                    data.setGrowth(0.0);

                if(counterVisit.getCounterType()!=null && counterVisit.getCounterType().equals(CounterType.DEALER) && !counterVisit.getSclCustomer().getGroups().contains(b2bUnitService.getUnitForUid(SclCoreConstants.B2B_UNIT.SCL_OTHER_UNIT_UID)))
                {
                    //journeydate
                    Double outstandingBefore = djpVisitDao.getOutstandingAmountBetweenDates(counterVisit.getSclCustomer().getCustomerNo(),orderStart,orderend);
                    //journeydate +1
                    Double outstandingAfter = djpVisitDao.getOutstandingAmountBetweenDates(counterVisit.getSclCustomer().getCustomerNo(),orderStart1,orderend1);

                    BaseSiteModel brand = baseSiteService.getCurrentBaseSite();
                    CustomerCategory category = CustomerCategory.TR;

                   Calendar cal = Calendar.getInstance();
                    cal.setTime(journeyStartDate);
                    cal.add(Calendar.YEAR, -1);//journey date on last date and  last year
                    Date prevYear = cal.getTime();
                    cal.add(Calendar.DATE, 1);//journey date on currentdate and current year
                    Date prevYear2 = cal.getTime();

                    //Source change confirmation needed for Dealer - Sales history
                   /* double averageDailySalesBefore = djpVisitDao.getAvgSalesDataForDealer(counterVisit.getSclCustomer().getCustomerNo(), orderStart, prevYear, category, brand)/365;
                    double averageDailySalesAfter = djpVisitDao.getAvgSalesDataForDealer(counterVisit.getSclCustomer().getCustomerNo(), orderStart1, prevYear2, category, brand)/365;
*/                  B2BCustomerModel customerModel=counterVisit.getSclCustomer();
                    double averageDailySalesBefore = sclSalesSummaryService.getSalesByMonth(customerModel,date1.getMonthValue(),date1.getYear()-1, (List<String>) null);
                    double averageDailySalesAfter = sclSalesSummaryService.getSalesByMonth(customerModel,date3.getMonthValue(),date3.getYear(), (List<String>) null);
    ...
    private void populateVisitDetailsPlanVsActual(MarketVisitDetailsData marketVisitDetailsData, VisitMasterModel visitMasterModel) {
        ...
        if(CollectionUtils.isNotEmpty(countersDetailsModelList)){
            for(CounterVisitMasterModel counterVisitMasterModel: countersDetailsModelList ){
                ...
                counterVisitedData.setCustomerName(null!= counterVisitMasterModel.getSclCustomer()?counterVisitMasterModel.getSclCustomer().getName():StringUtils.EMPTY);
                counterVisitedData.setCustomerCode(null!= counterVisitMasterModel.getSclCustomer()?counterVisitMasterModel.getSclCustomer().getUid():StringUtils.EMPTY);
                counterVisitedData.setCustomerType(getCustomerType(counterVisitMasterModel.getSclCustomer()));
                counterVisitedData.setCustomerNo(null!= counterVisitMasterModel.getSclCustomer()?counterVisitMasterModel.getSclCustomer().getCustomerNo():StringUtils.EMPTY);
    ...
    public SalesHistoryData getSalesHistoryDataFor360(String counterVisitId,List<String> subAreaList,List<String> districtList) {
        ...
        else if(sclCustomer.getGroups() !=null && sclCustomer.getGroups().contains(influencerGroup))
        {
       ...
       /* double lastMonthSales = djpVisitDao.getSalesHistoryDataFor360(customerCode, currentBaseSite, transactionType, lastMonth.getMonthValue(), lastMonth.getYear());
        ...
        if(sclCustomer.getGroups()!=null && sclCustomer.getGroups().contains(retailerGroup)) {
            ...
            if(customerCode!=null) {
                monthlySalesTarget = salesPerformanceService.getMonthlySalesTargetForRetailer(counterVisitModel.getSclCustomer().getUid(),baseSiteService.getCurrentBaseSite(),null);
    ...
    public List<MonthlySalesData> getLastSixMonthSalesForRetailer(String counterVisitId) {
        ...
        /*try {
            ...
            if(Objects.nonNull(counterVisitModel.getSclCustomer()))
            {

                String customerNo = counterVisitModel.getSclCustomer().getCustomerNo() != null ? counterVisitModel.getSclCustomer().getCustomerNo() : " ";
                ...
                for(int i=0;i<=6;i++) {
                    ...
                    String formattedMonth1 = monthFormat.format(date).concat("-").concat(String.valueOf(date.getYear()));
                    String formattedMonth2 = monthFormat.format(lastMonth).concat("-").concat(String.valueOf(lastMonth.getYear()));
                    ...
                    if (customerNo != null){
                        ...
                        currentMonthSalesTarget = salesPerformanceService.getMonthlySalesTargetForRetailer(counterVisitModel.getSclCustomer().getUid(),baseSiteService.getCurrentBaseSite(),null,formattedMonth1);
                        lastMonthSalesTarget = salesPerformanceService.getMonthlySalesTargetForRetailer(counterVisitModel.getSclCustomer().getUid(),baseSiteService.getCurrentBaseSite(),null,formattedMonth2);
                    ...
                    for (int i = 0; i <= 6; i++) {
                        ...
                        if (!(userService.getCurrentUser().getGroups().contains(userService.getUserGroupForUID(SclCoreConstants.CUSTOMER.RETAILER_USER_GROUP_UID)))) {
                            if (userService.getCurrentUser().getGroups().contains(userService.getUserGroupForUID(SclCoreConstants.CUSTOMER.DEALER_USER_GROUP_UID))) {
                                ...
                                salesQty = networkService.getSalesQuantityForRetailerByMonthYear(counterVisitModel.getSclCustomer(), currentMonth.getMonthValue(), currentMonth.getYear(), sclDealer, null);
                                currentMonthSale = salesQty.get("quantityInMT");
                                salesQty = networkService.getSalesQuantityForRetailerByMonthYear(counterVisitModel.getSclCustomer(), lastMonth.getMonthValue(), lastMonth.getYear(), sclDealer, null);
                                lastMonthSale = salesQty.get("quantityInMT");
                            } else {
                                salesQty = networkService.getSalesQuantityForRetailerByMonthYear(counterVisitModel.getSclCustomer(), currentMonth.getMonthValue(), currentMonth.getYear(), null, null);
                                currentMonthSale = salesQty.get("quantityInMT");
                                salesQty = networkService.getSalesQuantityForRetailerByMonthYear(counterVisitModel.getSclCustomer(), lastMonth.getMonthValue(), lastMonth.getYear(), null, null);
                                lastMonthSale = salesQty.get("quantityInMT");
                            }
                        } else {
                            salesQty = networkService.getSalesQuantityForRetailerByMonthYear(counterVisitModel.getSclCustomer(), currentMonth.getMonthValue(), currentMonth.getYear(), null, null);
                        ...
                        Date date = Date.from(currentMonth.atStartOfDay(ZoneId.systemDefault()).toInstant());
                        var monthYear = SclDateUtility.getFormattedDate(date, "MMM-YYYY");
                        Double targetSale = salesPerformanceDao.getMonthlySalesTargetForRetailer(counterVisitModel.getSclCustomer().getUid(), currentBaseSite, monthYear, null);
                        data.setTargetSales(targetSale);
                        if (targetSale > 0) {
                        ...
                        dataList.add(data);
                        LOG.info("Current sale:" + currentMonthSale);
                        LOG.info("Last Month sale:" + lastMonthSale);
                        LOG.info("Growth retailer sale:" + growth);
                        LOG.info("percentage sale:" + data.getPercentage());
                        currentMonth = lastMonth;
                    }
                }
            }
    ...
    @Override
    public CustomerSubAreaMappingModel createCustomerSubAreaMapping(String state, String district, String taluka, SclCustomerModel sclCustomer, CMSSiteModel site) {
        ...
        customerSubAreaMapping.setSclCustomer(sclCustomer);
        customerSubAreaMapping.setIsActive(Boolean.TRUE);
        customerSubAreaMapping.setBrand(site);
        if(sclCustomer.getCounterType()!=null) {
            customerSubAreaMapping.setCounterType(sclCustomer.getCounterType().getCode());
        }
        if(sclCustomer.getGroups()!=null && sclCustomer.getGroups().contains(userService.getUserGroupForUID(SclCoreConstants.B2B_UNIT.SCL_OTHER_UNIT_UID))) {
    ...
    public Boolean saveCustomerCoordinates(String customerId, Double latitude, Double longitude) {
        SclCustomerModel customer = sclCustomerService.getSclCustomerForUid(customerId);
    ...
    @Override
    public List<MonthlySalesData>  getLastSixMonthSalesForRetailer(List<MonthlySalesData> dataList, SclCustomerModel customer,List<SclCustomerModel> sclDealer) {
        BaseSiteModel currentBaseSite = baseSiteService.getCurrentBaseSite();
        LocalDate currentMonth = LocalDate.now();
        Map<String,Double> salesQty=new HashMap<>();
        for (int i = 1; i <= 6; i++) {
            MonthlySalesData data = new MonthlySalesData();
            salesQty = networkService.getSalesQuantityForRetailerByMonthYear(customer,currentMonth.getMonthValue(), currentMonth.getYear(),sclDealer,null);
            Double currentMonthSale=0.0;
            if(!(userService.getCurrentUser().getGroups().contains(userService.getUserGroupForUID(SclCoreConstants.CUSTOMER.RETAILER_USER_GROUP_UID)))) {
                     currentMonthSale = salesQty.get("quantityInMT");
                }else{
            ...
            //  Double currentMonthSale = salesPerformanceDao.getActualTargetSalesForSelectedMonthAndYearForRetailer(customer,currentBaseSite,currentMonth.getMonthValue(), currentMonth.getYear(),null);
            LocalDate lastMonth = currentMonth.minusMonths(1);
            salesQty=networkService.getSalesQuantityForRetailerByMonthYear(customer,lastMonth.getMonthValue(), lastMonth.getYear(),sclDealer,null);
            Double lastMonthSale=0.0;
            if(!(userService.getCurrentUser().getGroups().contains(userService.getUserGroupForUID(SclCoreConstants.CUSTOMER.RETAILER_USER_GROUP_UID)))) {
                lastMonthSale = salesQty.get("quantityInMT");
            }else{
     ...
     */
    @Override
    public Boolean isNonSclCounter(SclCustomerModel sclCustomerModel) {
        return Objects.nonNull(sclCustomerModel.getDefaultB2BUnit()) && sclCustomerModel.getDefaultB2BUnit().getUid().equalsIgnoreCase(SclCoreConstants.B2B_UNIT.SCL_OTHER_UNIT_UID);
    }
     ...
     */
    @Override
    public AddressModel getCustomerOwnAddress(SclCustomerModel sclCustomer) {
        if(CollectionUtils.isNotEmpty(sclCustomer.getAddresses())) {
         List<AddressModel> addressModels= sclCustomer.getAddresses().stream().filter(add -> (StringUtils.isNotEmpty(add.getSapAddressUsage()) && add.getSapAddressUsage().equalsIgnoreCase("DE")) && sclCustomer.getUid().equalsIgnoreCase(add.getPartnerFunctionId())).toList();
    ...
    /**
     *
     * @param sclCustomerModels
     * @return
     */
    @Override
    public List<SclCustomerModel> filterCustomerByDOTerritoryCode(List<SclCustomerModel> sclCustomerModels)
    {
        ...
        if (CollectionUtils.isNotEmpty(territoryMasterModels) && CollectionUtils.isNotEmpty(sclCustomerModels)) {
            ...
            sclCustomerModels.stream().forEach(sclCustomer -> {
                ...
                //dealer
                if((Objects.nonNull(sclCustomer) && Objects.nonNull(sclCustomer.getDefaultB2BUnit())) && sclCustomer.getDefaultB2BUnit().getUid().equalsIgnoreCase(SclCoreConstants.B2B_UNIT.SCL_SHREE_UNIT_UID)) {
                    if ((Objects.nonNull(sclCustomer.getCounterType()) && sclCustomer.getCounterType().equals(CounterType.DEALER)) && Objects.nonNull(sclCustomer.getTerritoryCode()))
                    {
                        ...
                        Integer retailerCount = getRetailerCountByTerritory(sclCustomer, territoryMasterModelList);
                    ...
                    //non-scl user
                    LOG.info(String.format("non scl user ::%s ",sclCustomer));
                    filterdList.add(sclCustomer); 
                }
    ...
    public  Map<List<BrandModel>,List<CompetitorProductModel>> getBrandsCompetitorProducts(List<String> brandIds) {
        List<BrandModel> brands=brandDao.findBrandProductById(brandIds);
        List<CompetitorProductModel> products =new ArrayList<>();
        SclUserModel sclUser =(SclUserModel) userService.getCurrentUser();
        List<String> states = new ArrayList<>();
        if(sclUser!=null && sclUser.getUserType()!=null && sclUser.getUserType().equals(SclUserType.TSO)){
            List<TsoTalukaMappingModel> tsoTalukaMappingForTso = geographicalRegionDao.getTsoTalukaMappingForTso(sclUser);
        ...
        products = competitorProductDao.getListOfCompetitorProductsByState(brands, states);
        Map<List<BrandModel>,List<CompetitorProductModel>> brandProductsMap = new HashMap();
        if(products != null && !products.isEmpty())
        {
            brandProductsMap.put(brands, products);
        }
        for(Map.Entry<List<BrandModel>, List<CompetitorProductModel>> brandProductMap :brandProductsMap.entrySet()){
            LOG.info("Key: " + brandProductMap.getKey() + ", Value: " + brandProductMap.getValue());
        }
     ...
     */
    @Override
    public List<MarketMappingDetailsModel> getMarketMappingDetails(String uid) {
        if (StringUtils.isNotEmpty(uid)) {
            SclUserModel sclUser = (SclUserModel) userService.getCurrentUser();
            CounterVisitMasterModel latestCounterVisit = djpVisitDao.getLatestCounterVisit(uid, sclUser);
            if(Objects.nonNull(latestCounterVisit)){
            return djpVisitDao.getMarketMappingDetailsByVisitId(latestCounterVisit);
            }
        }
       return  Collections.emptyList();
    }
    ...
    public  Map<List<BrandModel>,List<CompetitorProductModel>> getBrandsCompetitorProductsByUid(String uid) {
        Map<List<BrandModel>, List<CompetitorProductModel>> brandProductsMap = new HashMap();
        List<BrandModel> selectedBrands = new ArrayList<>();
        List<CompetitorProductModel> selectedProducts = new ArrayList<>();
        List<String> states = new ArrayList<>();
        if(StringUtils.isNotEmpty(uid)) {
            SclUserModel sclUser = (SclUserModel) userService.getCurrentUser();
            CounterVisitMasterModel latestCounterVisit = djpVisitDao.getLatestCounterVisit(uid, sclUser);
            ...
            if (Objects.nonNull(latestCounterVisit)) {
                selectedBrands = brandDao.selectedBrands(latestCounterVisit);
                selectedProducts = competitorProductDao.getCompetitorProductByVisitId(latestCounterVisit);
                LOG.info(String.format("Get Latest Counter Visit Master Id :%s",latestCounterVisit.getId()));
            }
            if (CollectionUtils.isNotEmpty(selectedProducts) || CollectionUtils.isNotEmpty(selectedBrands)) {
                brandProductsMap.put(selectedBrands, selectedProducts);
            }
            for(Map.Entry<List<BrandModel>, List<CompetitorProductModel>> brandProductMap :brandProductsMap.entrySet()){
                LOG.info("Key: " + brandProductMap.getKey() + ", Value: " + brandProductMap.getValue());
            }
