	...
	public boolean validateOrder(OrderModel order) {
		...
		if(order.getCrmOrderType()==null || !order.getCrmOrderType().equals(CRMOrderType.GIFT)) {
			...
			if(order.getRouteId()!=null) {
				SalesOrderDeliverySLAModel deliverySlaModel = sclSalesOrderDeliverySLADao.findByRoute(order.getRouteId());
				...
				if(entry.getExpectedDeliveryDate()!=null && entry.getCalculatedDeliveryDate()!=null) {
					...
					if(expectedDeliveryDate.isBefore(calculatedDeliveryDate)) {
						...
						try {
							...
							sclNotificationService.submitOrderNotification(order,so,body,sub,NotificationCategory.DELIVERY_WINDOW_NOT_MET);
							sclNotificationService.submitOrderNotification(order, (B2BCustomerModel) order.getUser(),body,sub,NotificationCategory.DELIVERY_WINDOW_NOT_MET);
						}
						catch(Exception e) {
					...
					if(expectedDeliveryDate.isEqual(calculatedDeliveryDate)) {
						int expectedSlot = deliverySlotMasterDao.findByEnum(entry.getExpectedDeliveryslot().getCentreTime()).getSequence();
...
//			if(order.getTotalPrice()!=null) {
			Double totalCreditLimit = djpVisitDao.getDealerCreditLimit(((SclCustomerModel)order.getUser()).getCustomerNo());
			Double currentOutstanding  = djpVisitDao.getDealerOutstandingAmount(((SclCustomerModel)order.getUser()).getCustomerNo());
			...
			if((checkAmount)>totalCreditLimit)
			{
				...
				Double billingPrice = order.getEntries().get(0).getBasePrice()!=null?order.getEntries().get(0).getBasePrice():0.0;//sclOrderService.getBillingPriceForProduct(order.getSite(), product.getInventoryId(), order.getDeliveryAddress().getErpCity(), CustomerCategory.TR, product.getBagType(), product.getState());
				int qty = 0 ;
				...
				if(isFirstTime) {
					if(((SclCustomerModel)order.getUser()).getIsCreditLimitBreached()==null || !((SclCustomerModel)order.getUser()).getIsCreditLimitBreached()) {
						((SclCustomerModel)order.getUser()).setIsCreditLimitBreached(Boolean.TRUE);
						((SclCustomerModel)order.getUser()).setCreditLimitBreachedDate(new Date());
						modelService.save(order.getUser());
					}

					//Call Credit Limit Breach In App Notification
					//sclNotificationService.sendInAppNotificationForCreditLimitBreach(order,totalCreditLimit,currentOutstanding,orderAmount,pendingOrderAmount);
					try {
...
//				else if(maxOrderQtyBreachedPercentage>25){
			...
			if(!isFirstTime && maxApprovelLevelNumber<=approvedLevel) {
				...
				SclWorkflowModel approvalWorkflowModel = null;
				if(order.getApprovalWorkflow()==null)
				{
					approvalWorkflowModel= sclWorkflowService.saveWorkflow("ORDER_APPROVAL_WORKFLOW", WorkflowStatus.START, WorkflowType.ORDER);
				...
				sclWorkflowService.saveWorkflowAction(approvalWorkflowModel, "", order.getSite(), order.getSubAreaMaster(), approvalLevel);
				return true;
			}
		}
		else if(!isFirstTime){
	...
	private void validateMaxQuatity(OrderModel order, List<String> rejectionReasons, Map<String,String> suggestions) {
		...
		//		double maxInvoicedQty = orderValidationProcessDao.getMaxQuantityThreshold(order.getSite(), (SclCustomerModel)order.getUser(), product);
		...
		//			double deliveryDateQty = orderValidationProcessDao.getQtyForExpectedDeliveredDate(order.getSite(), (SclCustomerModel)order.getUser(), product, null);
		...
		double maxThresholdQty = maxInvoicedThresholdQty>minMaxThresholdQty?maxInvoicedThresholdQty:minMaxThresholdQty;
		Double orderQty = order.getEntries().stream().collect(Collectors.summingDouble(each -> each.getQuantityInMT()));
		double deliveryDateQty = orderValidationProcessDao.getQtyForExpectedDeliveredDate(order.getSite(), (SclCustomerModel)order.getUser(), product, order.getEntries().get(0).getExpectedDeliveryDate());
		if((deliveryDateQty+ orderQty) > maxThresholdQty) {
