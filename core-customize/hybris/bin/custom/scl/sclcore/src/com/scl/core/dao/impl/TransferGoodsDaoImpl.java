...
public class TransferGoodsDaoImpl implements TransferGoodsDao{
	...
	public List<SclUserModel> searchForSalesOfficers(String key, List<String> officerCodes) {
		...
		final StringBuilder builder = new StringBuilder("SELECT {pk} FROM {SclUser} WHERE {employeeCode} IN (?officerCodes) AND {name} LIKE ?searchKey AND {email} LIKE ?searchKey  AND {mobileNumber} LIKE ?searchKey AND {employeeCode} LIKE ?searchKey");
