...
import com.scl.facades.data.warehousing.WarehouseListData;
...
import com.scl.occ.dto.ISOVisibilityListWsDTO;
import com.scl.occ.dto.djp.VisitMasterListWsDTO;
import com.scl.occ.dto.warehousing.WarehouseListWsDto;
...
import com.scl.facades.data.ISOVisibilityListData;
import com.scl.facades.data.VisitMasterData;
import com.scl.facades.data.VisitMasterListData;
...
@PermitAll
public class DepotOperationsController extends SclBaseController
{
	@Resource
    ...
    public ResponseEntity<List<DepotStockData>> getStockAvailability(
    		@Parameter(description = "List of Product Grade") @RequestParam(required = false) final List<String> productCodes,@Parameter(description = "depotCode") @RequestParam(required = false) final List<String> depotCode)
    {
		List<DepotStockData> stock = depotOperationsFacade.getStockAvailability(productCodes, depotCode);
    ...
    public ResponseEntity<DailyCapacityUtilizationData> getDailyCapacityUtilization(
    		@Parameter(description = "List of Product Code") @RequestParam(required = false) final List<String> productCodes,
    		@Parameter(description = "depotCode") @RequestParam(required = false) final List<String> depotCode)
    {
        return ResponseEntity.status(HttpStatus.OK).body(depotOperationsFacade.getDailyCapacityUtilization(productCodes, depotCode));
    }
    ...
    @ResponseBody
    @ApiBaseSiteIdAndUserIdParam
    public List<DropdownData> getDepotListOfGrades(@RequestParam(required = false) boolean forISOOrders)
    {
		return depotOperationsFacade.getDepotListOfGrades(forISOOrders);
    }
	...
	public ISOVisibilityListWsDTO getISOVisibilityDetails(@Parameter(description = "Optional pagination parameter. Default value 0.") @RequestParam(defaultValue = DEFAULT_CURRENT_PAGE) final int currentPage,
			...
			@Parameter(description="ROAD or RAIL", required = false) @RequestParam(required = false) final List<String> deliveryMode, @Parameter(description = "products",required = false) @RequestParam(required = false) final List<String> productCodes,
			@Parameter(description="depots", required = false) @RequestParam(required = false) final List<String> depots)
	{
        //This is Partial Changes As Per business Requirement
		...
		SearchPageData<ISOVisibilityData> isoVisibilityDetails = depotOperationsFacade.getISOVisibilityDetails(searchPageData, mrnStatus, depots, deliveryMode, productCodes);
    ...
    @RequestMapping(value="/productsListForDepot", method = RequestMethod.GET)
    @ResponseBody
    @Operation(operationId = "getProductListForDepot", summary = "Product List for Depot", description = "Get Product List for Depots")
    @ApiBaseSiteIdAndUserIdParam
    public ResponseEntity<List<DepotProductData>> getProductListForDepot( @RequestParam(required = false) boolean forISOOrders)
    {
        List<DepotProductData> productListForDepot = depotOperationsFacade.getProductListForDepot(forISOOrders);
        return ResponseEntity.status(HttpStatus.OK).body(productListForDepot);
    }
