...
@PermitAll
public class OrderHistoryController extends SclBaseController
{
	@Autowired  
	...
	@ApiBaseSiteIdAndUserIdParam
    public SclOrderHistoryListWsDTO getTradeOrderListing(@Parameter(description = "sourceType") @RequestParam(required = false) final String sourceType,@Parameter(description = "filter") @RequestParam(required = false) final String filter,
    ...
    {	
		...
		SearchPageData<SclOrderHistoryData> entries = orderHistoryFacade.getTradeOrderListing(searchPageData, sourceType, filter, month1, year1,productName,orderType,status);
		
		sclOrderHistoryListData.setOrdersList(entries.getResults());
