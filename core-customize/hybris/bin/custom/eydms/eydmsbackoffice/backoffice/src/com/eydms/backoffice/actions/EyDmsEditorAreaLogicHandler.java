package com.eydms.backoffice.actions;
...
public class EyDmsEditorAreaLogicHandler extends BackofficeEditorAreaLogicHandler{
            ...
            throws ObjectSavingException
    {
		...
		if(currentObject.getClass().equals(OrderModel.class)) {
			...
			B2BUnitModel shree = (B2BUnitModel) b2bUnitService.getUnitForUid(EyDmsCoreConstants.B2B_UNIT.EYDMS_SHREE_UNIT_UID);
			B2BUnitModel bangur = (B2BUnitModel) b2bUnitService.getUnitForUid(EyDmsCoreConstants.B2B_UNIT.EYDMS_BANGUR_UNIT_UID);
			B2BUnitModel rockstrong = (B2BUnitModel) b2bUnitService.getUnitForUid(EyDmsCoreConstants.B2B_UNIT.EYDMS_ROCKSTRONG_UNIT_UID);
			...
			if(order.getUser()!=null)
			{
				...
				{
					site = baseSiteService.getBaseSiteForUID(EyDmsCoreConstants.SITE.SHREE_SITE);
				}
				else if(order.getUser().getGroups().contains(bangur))
				{
					site = baseSiteService.getBaseSiteForUID(EyDmsCoreConstants.SITE.BANGUR_SITE);
				}
				else if(order.getUser().getGroups().contains(rockstrong))
				{
					site = baseSiteService.getBaseSiteForUID(EyDmsCoreConstants.SITE.ROCKSTRONG_SITE);
				}
