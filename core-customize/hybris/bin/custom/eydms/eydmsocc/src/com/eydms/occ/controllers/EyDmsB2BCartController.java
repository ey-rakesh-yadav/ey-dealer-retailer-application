...
@Controller
@ApiVersion("v2")
@Tag(name = "EYDMS B2B Carts")
public class EyDmsB2BCartController extends EyDmsBaseController {
	...
	@RequestMappingOverride(priorityProperty = "eydmsocc.CartResource.addCartEntry.priority")
	...
	public EyDmsOrderHistoryListWsDTO getCarts(@ApiFieldsParam @RequestParam(defaultValue = DEFAULT_FIELD_SET) final String fields, 
			...
			@Parameter(description = "Filters by Month") @RequestParam(required = false) Integer month, @Parameter(description = "Filters by Year") @RequestParam(required = false) Integer year, @RequestParam(required = false) final String filter)
	{
		...
		final SearchPageData<EyDmsOrderHistoryData> eydmsOrderHistoryData = eydmsB2BCartFacade.getSavedCartsBySavedBy(searchPageData, filter, month1, year1,productName,orderType);
		eydmsOrderHistoryListData.setOrdersList(eydmsOrderHistoryData.getResults());
		
		if (eydmsOrderHistoryData.getPagination() != null)
		{
			response.setHeader(HEADER_TOTAL_COUNT, String.valueOf(eydmsOrderHistoryData.getPagination().getTotalNumberOfResults()));
		}
