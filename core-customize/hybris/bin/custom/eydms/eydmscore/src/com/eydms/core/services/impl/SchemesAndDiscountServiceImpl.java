...
//		}
	...
	public Double getTotalAvailablePoints(String customerCode) {
		...
		if(user!=null && user instanceof EyDmsCustomerModel && ((EyDmsCustomerModel)user).getAvailablePoints()!=null)
		{
			totalAvailablePoints = ((EyDmsCustomerModel)user).getAvailablePoints();
		}
		return totalAvailablePoints;
	}

	@Override
	public synchronized Boolean updateInfluencerPoint(EyDmsCustomerModel customer, double redeemPoint, TransactionType transactionType, GiftSchemeModel sheme, String orderCode) {
		customer = (EyDmsCustomerModel)userService.getUserForUID(customer.getUid());
		...
		try {
			...
			for (EyDmsUserModel tsoUser : tsoList) {
				eydmsNotificationService.submitDealerNotification((B2BCustomerModel) tsoUser, body, sub, NotificationCategory.REDEEM_REQUEST, suggestion);
			}
	...
	public synchronized Boolean updateInfluencerPoint(String orderCode) {
		...
		return updateInfluencerPoint((EyDmsCustomerModel)order.getUser(), points, TransactionType.DEBIT, null, order.getCode());
	}
	...
	public Boolean rejectRedeemRequest(String requestId, String rejectionReason) {
		...
		if(transactionModel!=null) {
			...
			EyDmsCustomerModel influencer =  (EyDmsCustomerModel) order.getUser();
			Double points = 0.0;
...
//				points = order.getEntries().get(0).getQuantity();
//			}			
//			updateInfluencerPoint((EyDmsCustomerModel) userService.getCurrentUser(), points, TransactionType.CREDIT, order.getGiftShop(), order.getCode());
			modelService.save(order);
		}
		return Boolean.TRUE;
	}
