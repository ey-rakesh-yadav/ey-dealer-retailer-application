...
public class EYDMSNewNetworkServiceImpl implements EYDMSNewNetworkService {
   ...
   NetworkDao networkDao;
    ...
    public List<List<Object>> getDealerDetailedSummaryList(RequestCustomerData requestCustomerData) {
        return eydmsOrderCountDao.getDealerDetailedSummaryList(requestCustomerData);
    }

    @Override
    public List<NetworkAdditionPlanModel> getProposedPlanSummaryList(LeadType leadType,List<EyDmsUserModel> soForUser) {
    ...
    public boolean updateTargetStatusForApprovalNwAddition(EYDMSNetworkAdditionPlanData salesApprovalData) {
       ...
       /* eydmsUserModel=salesApprovalData.getRaisedBy();
        EyDmsUserModel eydmsUserModel= (EyDmsUserModel) userService.getUserForUID(salesApprovalData.getRaisedByUid());
        ...
        SubAreaMasterModel subAreaMaster= networkDao.getSubareaForSOString(subArea);
                networkAdditionPlanModel = networkDao.getNetworkAdditionPlan(eydmsUserModel);*/
                ...
                if(currentUser.getUserType()!=null && currentUser.getUserType().equals(EyDmsUserType.TSM)) {
                    try {
                        ...
                        if (networkAdditionPlanModel.getApprovalWorkflow() != null) {
                            EyDmsWorkflowActionModel eydmsWorkflowActionModel = networkAdditionPlanModel.getApprovalWorkflow().getCurrent();


                            eydmsWorkflowService.updateWorkflowAction(eydmsWorkflowActionModel, currentUser, WorkflowActions.APPROVED, "NetworkAddition is approved");
   ...
   @Override
    public boolean targetSendForRevisionNwAddition(ProposePlanData salesRevisedTargetData) {
       ...
       networkAdditionPlanModel = networkDao.getNetworkAdditionPlan(salesRevisedTargetData.getId());
                ...
                if(networkAdditionPlanModel!=null) {
                    eydmsWorkflow = networkAdditionPlanModel.getApprovalWorkflow();
                }
                ...
                if(currentUser.getUserType()!=null && currentUser.getUserType().equals(EyDmsUserType.TSM))
                {
                    try {
                          ...
                          if (networkAdditionPlanModel.getApprovalWorkflow() != null) {
                            ...
                            eydmsWorkflowService.updateWorkflowAction(eydmsWorkflowActionModel, currentUser, WorkflowActions.REVISED, "NetworkAddition is revised");
    ...
    @Override
    public SearchPageData<NetworkAdditionPlanModel> getProposedPlanSummaryListForTSMRH(SearchPageData searchPageData,LeadType leadType,List<String> statuses,boolean isPendingForApproval,List<EyDmsUserModel> soForUser, List<EyDmsUserModel> tsmForUser,EyDmsUserModel currentUser) {
