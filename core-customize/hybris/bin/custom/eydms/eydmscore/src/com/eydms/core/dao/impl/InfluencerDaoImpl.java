...
public class InfluencerDaoImpl implements InfluencerDao{
	...
	Map<String, String> eydmsCustomerSortCodeToQueryAlias;
	...
	public Map<String, String> getEyDmsCustomerSortCodeToQueryAlias() {
		return eydmsCustomerSortCodeToQueryAlias;
	}

	public void setEyDmsCustomerSortCodeToQueryAlias(Map<String, String> eydmsCustomerSortCodeToQueryAlias) {
		this.eydmsCustomerSortCodeToQueryAlias = eydmsCustomerSortCodeToQueryAlias;
	}

	@Override
	public EyDmsUserModel searchSalesOfficer(List<String> keys) {
		final Map<String, Object> params = new HashMap<String, Object>();
		final StringBuilder builder = new StringBuilder("SELECT {pk} FROM {EyDmsUser} WHERE {email} IN (?keys) OR {name} IN (?keys) OR {mobileNumber} IN (?keys) OR {uid} IN (?keys) ");
	...
	@Override
	public EyDmsCustomerModel searchEyDmsCustomer(List<String> keys) {
		final Map<String, Object> params = new HashMap<String, Object>();
		final StringBuilder builder = new StringBuilder("SELECT {pk} FROM {EyDmsCustomer} WHERE {email} IN (?keys) OR {name} IN (?keys) OR {mobileNumber} IN (?keys) or {uid} in (?keys)");
		params.put("keys", keys);
		final FlexibleSearchQuery query = new FlexibleSearchQuery(builder.toString());
	...
	public List<List<Object>> getInfluencerTypeList(String influencerType,List<SubAreaMasterModel> subAreaMasterList) {
		...
		StringBuilder builder = new StringBuilder("SELECT {e.code},count({c.pk}) FROM {").append(TerritoryManagementDaoImpl.CUSTOMER_FOR_USER_QUERY)
				...
				.append(EyDmsDateUtility.getLastXDayQuery("c.dateOfJoining", params, 30));
	...
	public List<List<Object>> getInfluencerNetworkAddition(String influencerType, List<SubAreaMasterModel> subAreaMasterList) {
		...
		StringBuilder builder = new StringBuilder("SELECT Year({c.dateOfJoining}),MONTH({c.dateOfJoining}),count({c.pk}) FROM {").append(TerritoryManagementDaoImpl.CUSTOMER_FOR_USER_QUERY)
				...
				.append(EyDmsDateUtility.getCurrentPreviousMonthClauseQuery("c.dateOfJoining", params));
	...
	public Integer getOnboardingsPendingForApproval(String influencerType, List<SubAreaMasterModel> subAreaMasterList) {
		...
		StringBuilder builder = new StringBuilder("select count({c:pk}) from {EyDmsCustomer as c join CustomerSubAreaMapping as m on {m.eydmsCustomer}={c.pk} } ")
	...
	public SearchPageData<EyDmsCustomerModel> getInfluencerOnboardingList(SearchPageData searchPageData,
		...
		final Map<String, Object> params = new HashMap<String, Object>();
		StringBuilder builder = new StringBuilder("select {c:pk} from {EyDmsCustomer as c join CustomerSubAreaMapping as m on {m.eydmsCustomer}={c.pk} } ")
		...
		{
			builder.append(EyDmsDateUtility.getDateRangeClauseQuery("c.applicationDate", customerRequestData.getStartDate(), customerRequestData.getEndDate(), params));
		}
		else {
			builder.append(EyDmsDateUtility.getMtdClauseQuery("c.applicationDate", params));
		}
		...
		parameter.setSortCodeToQueryAlias(getEyDmsCustomerSortCodeToQueryAlias());
	...
	public SearchPageData<LeadMasterModel> getPaginatedLeadList(SearchPageData searchPageData, LeadRequestData leadRequestData) {
		...
		if(StringUtils.isNotBlank(leadRequestData.getStartDate()) && StringUtils.isNotBlank(leadRequestData.getEndDate())){
			builder.append(EyDmsDateUtility.getDateRangeClauseQuery("l:creationtime",leadRequestData.getStartDate(), leadRequestData.getEndDate(),params));
		}
		else
		{
			builder.append(EyDmsDateUtility.getMtdClauseQuery("l:creationtime",params));
		}
	...
	public SearchPageData<MeetingScheduleModel> getPaginatedScheduleMeetList(SearchPageData searchPageData, String meetingType, String startDate, String endDate, String searchFilter, List<String> status) {
		...
		if(currentUser instanceof EyDmsUserModel) {
			...
			builder.append("select {m.pk} from {MeetingSchedule as m join MeetingScheduleToCustomerRel as r on {m.pk}={r.source} join EyDmsCustomer as s on {r.target}={s.pk}}  where {r.target} in (?currentUser) ");
		}
		...
		if(startDate!=null && endDate!=null){
			builder.append(" and ");
			builder.append(EyDmsDateUtility.getDateRangeClauseQuery("m:eventDate",startDate, endDate,params));
		}
		else
		{
			builder.append(" and ");
			builder.append(EyDmsDateUtility.getMtdClauseQuery("m:eventDate",params));
		}
	...
	public Integer getVisitCountMTD(EyDmsCustomerModel eydmsCustomer, int month, int year) {
		final Map<String, Object> params = new HashMap<String, Object>();
		final StringBuilder builder = new StringBuilder("SELECT COUNT({endVisitTime}) FROM {InfluencerVisitMaster AS c} WHERE {c.eydmsCustomer}=?eydmsCustomer AND {c.endVisitTime} IS NOT NULL AND ").append(EyDmsDateUtility.getDateClauseQueryByMonthYear("c.endVisitTime", month, year, params));
	...
	@Override
	public Integer getInfLeadGeneratedCount(EyDmsCustomerModel eydmsCustomer) {
		final Map<String, Object> params = new HashMap<String, Object>();
		Integer lastXDays = dataConstraintDao.findDaysByConstraintName("PAST_YEAR_LEADS");
		LocalDate lastYear = LocalDate.now().minusYears(lastXDays);
		...
		if(currentUser.getUserType()!=null) {
			if (currentUser.getUserType().getCode().equalsIgnoreCase("TSO")) {
				builder.append("SELECT COUNT({l:pk}) FROM {LeadMaster as l join InfluencerVisitMaster as c on {l:counterVisit}={c:pk}} WHERE {l:counterType} in (?counterType) and {l:status}=?leadStatus and {l:creationtime} >=?startDate and {l:creationtime} <?endDate and {c:eydmsCustomer}=?eydmsCustomer");
				counter.add(CounterType.INFLUENCER);
				counter.add(CounterType.SITE);
				params.put("counterType", counter);
			} else {
				builder.append("SELECT COUNT({l:pk}) FROM {LeadMaster as l join InfluencerVisitMaster as c on {l:counterVisit}={c:pk}} WHERE {l:counterType}=?counterType and {l:status}=?leadStatus and {l:creationtime} >=?startDate and {l:creationtime} <?endDate and {c:eydmsCustomer}=?eydmsCustomer");
				params.put("counterType", CounterType.INFLUENCER);
			}
		}
		else {
