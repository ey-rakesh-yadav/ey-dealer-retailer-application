...
import com.eydms.core.enums.DetailsOfTaskToBeDone;
import com.eydms.core.enums.ModeOfMeeting;
import com.eydms.core.enums.SiteRootCause;
import com.eydms.core.enums.TAServiceRequestStatus;
import com.eydms.core.enums.TATSOStatus;
import com.eydms.core.enums.TESuppportRequired;
import com.eydms.core.enums.TypeOfCement;
import com.eydms.core.jalo.ComplaintRootCauseIdentified;
import com.eydms.core.jalo.ComplaintTEMeeting;
...
public class TechnicalAssistanceFacadeImpl implements TechnicalAssistanceFacade{
	...
	private Converter<EyDmsCustomerModel, CustomerData> dealerBasicConverter;
	...
	@Resource
	private Converter<ComplaintDispatchDetailsModel,ComplaintDispatchDetailsData> eydmsComplaintDispatchDetailsConverter;
	...
	public SearchPageData<EndCustomerComplaintData> getEndCustomerComplaints(SearchPageData searchPageData, String startDate, String endDate, String partnerCode
		...
		SearchPageData<EndCustomerComplaintModel> searchResult = technicalAssistanceService.getEndCustomerComplaints(searchPageData, startDate, endDate, partnerCode, requestNo, filters,requestStatuses,isSiteVisitRequired,plannedVisitForToday, subAreaMasterPk);
		if(searchResult!=null && searchResult.getResults()!=null) {
			...
			list.forEach(entry -> {
				...
				if (entry.getTsoStatus()!=null && userService.getCurrentUser().getGroups()!=null && userService.getCurrentUser().getGroups().contains(userService.getUserGroupForUID(EyDmsCoreConstants.CUSTOMER.TSO_GROUP_ID))) {
				...
				if(entry.getRequestRaisedDate()!=null) {
					...
					DateFormat timeFormat = new SimpleDateFormat("HH:mm:ss"); // Customize the format as needed
	...
	public SearchPageData<TechnicalAssistanceData> getTechnicalAssistances(SearchPageData searchPageData, String startDate, String endDate, String name
		...
		SearchPageData<TechnicalAssistanceModel> searchResult = technicalAssistanceService.getTechnicalAssistances(searchPageData, startDate, endDate, name, requestNo, filters,status, constructionAdvisory);
	...
	public TechnicalAssistanceData getTechnicalAssistanceRequestDetails(String requestNo) {
		...
		if (model.getTsoStatus()!=null && userService.getCurrentUser().getGroups()!=null && userService.getCurrentUser().getGroups().contains(userService.getUserGroupForUID(EyDmsCoreConstants.CUSTOMER.TSO_GROUP_ID))) {
  ...
  @Override
	public DealerListData getAllRetailersForMaterial(String userId) {
		...
		List<EyDmsCustomerModel> retailersList = getTechnicalAssistanceService().getAllRetailersForMaterial(userId);
	...
	public DealerListData getAllDealersForMaterial(String userId) {
		List<EyDmsCustomerModel> dealersList = getTechnicalAssistanceService().getAllDealersForMaterial(userId);
	...
	public CustomerData getCustomerByCode(String customerCode) {
		if(Objects.nonNull(customerCode)){
			EyDmsCustomerModel eydmsCustomerModel = getEyDmsCustomerService().getEyDmsCustomerForUid(customerCode);
			CustomerData customerData =  getDealerBasicConverter().convert(eydmsCustomerModel);
			return customerData;
		}
	...
	public CustomerComplaintData getCustomerAssistanceRequestDetails(String requestNo) {
		...
		eydmsAddressPopulator.populate(model.getAddress(), address);
	...
	public EndCustomerComplaintData populateEndCustomerComplaintModel(EndCustomerComplaintModel model, EndCustomerComplaintData data, boolean isUIRelatedChange){
		...
		if(model!=null){
					...
					if (model.getProduct() != null) {
						data.setProduct(model.getProduct().getName());
					}
					...
					if(model.getRaisedBy()!=null) {
						...
						if(soTaggedname==null && model.getRaisedBy().getGroups()!=null && model.getRaisedBy().getGroups().contains(userService.getUserGroupForUID(EyDmsCoreConstants.CUSTOMER.SALES_OFFICER_GROUP_ID))) {
				...
				BaseSiteModel brand = model.getBrand();
...
//		trackingDataList.add(data2);
	...
	//		complaintDispatchDetailsModel.setTypeOfCement(TypeOfCement.valueOf(complaintDispatchDetailsData.getTypeOfCement()));
			...
			if(complaintDispatchDetailsData.getTypeOfCement()!=null){
				complaintDispatchDetailsModel.setSiteCementType(siteManagementDao.findCementTypeByCode(complaintDispatchDetailsData.getTypeOfCement()));
			}
	...
	public EndCustomerComplaintData submitNextVisitPlanDetailes(ScheduleNextVisitData scheduleNextVisitData) {
		...
		if(endCustomerComplaintModel!=null){
			...
			if(scheduleNextVisitData.getSiteId()!=null) {
				scheduleNextVisitModel.setSite((EyDmsSiteMasterModel)userService.getUserForUID(scheduleNextVisitData.getSiteId()));
	...
	public CustomerComplaintHomePageData countOfAssignedTicketNumbers() {
		...
		data.setOpenComplaintsCount(technicalAssistanceService.countOfAssignedTicketNumbers());
		return  data;
	}
	...
	//	List<ComplaintDispatchDetailsModel> sortedDispatchDetails = new ArrayList<>();
		if(model!=null){
			...
			dataList = eydmsComplaintDispatchDetailsConverter.convertAll(sortedDispatchDetails);
		}
		return dataList;
	}
